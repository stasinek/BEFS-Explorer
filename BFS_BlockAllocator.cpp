...
BlockAllocator::_Initialize(BlockAllocator *allocator)
{
	...
	uint64 usedBlocks = volume->NumBlocks() - freeBlocks;
...
status_t
BlockAllocator::Allocate(Transaction &transaction, Inode *inode, uint64 numBlocks,
...
bool
BlockAllocator::_CheckBitmapIsUsedAt(uint64 block) const
{
	size_t size = BitmapSize();
...
void
BlockAllocator::_SetCheckBitmapAt(uint64 block)
{
	size_t size = BitmapSize();
	uint32 index = block / 32;	// 32bit resolution
	if (index > size / 4)
...
BlockAllocator::CheckBlockRun(block_run run, const char *type, check_control *control, bool allocated)
{
	...
	int32 length = 0;
	uint64 firstMissing = -1, firstSet = -1;
	uint64 firstGroupBlock = (uint64)run.AllocationGroup() << fVolume->AllocationGroupShift();
