...
class DeviceOpener {
	public:
		...
		void *InitCache(uint64 numBlocks, uint32 blockSize);
		...
		status_t GetSize(uint64 *_size, uint32 *_blockSize = NULL);
...
disk_super_block::IsValid()
{
	...
	if ((Magic1() != (int32)SUPER_BLOCK_MAGIC1 && Magic1() != (int32)BFS_SUPER_BLOCK_MAGIC1)
		...
		|| NumBlocks() < 10
		|| AllocationGroups() != divide_roundup((int64)NumBlocks(),
...
//TODO, this should be strlcpy
	...
	uint64 bitmapBlocks = (numBlocks + bitsPerBlock - 1) / bitsPerBlock;
	...
	uint32 flags)
{
	...
	uint64 numBlocks = deviceSize / blockSize;
