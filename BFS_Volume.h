...
class Volume {
	public:
		...
		uint64				NumBlocks() const { return fSuperBlock.NumBlocks(); }
		uint64				UsedBlocks() const { return fSuperBlock.UsedBlocks(); }
		uint64				FreeBlocks() const { return NumBlocks() - UsedBlocks(); }
		...
		uint64				ToOffset(block_run run) const { return /*fStartPos.QuadPart+*/ (ToBlock(run) << BlockShift()); }
		uint64				ToBlock(block_run run) const { return ((((uint64)run.AllocationGroup()) << AllocationGroupShift()) | (uint64)run.Start()); }
		block_run			ToBlockRun(uint64 block) const;
		status_t			ValidateBlockRun(block_run run);

		uint64				ToVnode(block_run run) const { return ToBlock(run); }
		uint64				ToVnode(uint64 block) const { return block; }
		uint64				VnodeToBlock(vnode_id id) const { return (uint64)id; }
		...
		status_t			Allocate(Transaction &transaction, Inode *inode,
								uint64 numBlocks, block_run &run, uint16 minimum = 1);
		...
		status_t			Sync();
		Journal				*GetJournal(uint64 refBlock) const;
		...
		uint64				StartPos() const {return fStartPos.QuadPart; }
...
inline status_t
Volume::Allocate(Transaction &transaction, Inode *inode, uint64 numBlocks, block_run &run, uint16 minimum)
{
...
inline Journal *
Volume::GetJournal(uint64 /*refBlock*/) const
{
	 return fJournal;
}
