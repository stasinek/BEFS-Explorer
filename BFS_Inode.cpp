	...
	bool hasIndex, Index *index)
{
	...
	if (index != NULL) {
		...
		if ((hasIndex || fVolume->CheckForLiveQuery(name))
			...
			uint64 length = BPLUSTREE_MAX_KEY_LENGTH;
...
*/
status_t
Inode::ReadAttribute(const char *name, int32 type, uint64 pos, uint8 *buffer,
	uint64 *_length)
{
	if (pos < 0)
		pos = 0;
...
Inode::WriteAttribute(Transaction &transaction, const char *name, int32 type,
	uint64 pos, const uint8 *buffer, size_t *_length)
{
...
*/
status_t
Inode::FindBlockRun(uint64 pos, block_run &run, uint64 &offset)
{
	...
	if (data->MaxDirectRange() > 0 && pos >= data->MaxDirectRange()) {
		if (data->MaxDoubleIndirectRange() > 0 && pos >= data->MaxIndirectRange()) {
			...
			uint64 start = pos - data->MaxIndirectRange();
			...
			uint64 runBlockEnd = data->MaxDirectRange();

			CachedBlock cached(fVolume);
			uint64 block = fVolume->ToBlock(data->indirect);
...
status_t
Inode::ReadAt(uint64 pos, uint8 *buffer, uint64 *_length)
{
#if 0   //disable code
...
status_t
Inode::WriteAt(Transaction &transaction, uint64 pos, const uint8 *buffer,
	size_t *_length)
{
...
*/
status_t
Inode::FillGapWithZeros(uint64 pos, uint64 newSize)
{
	...
	while (length > 0) {
		...
		CachedBlock cached(fVolume);
		uint64 blockNumber = fVolume->ToBlock(run);
...
Inode::_AllocateBlockArray(Transaction &transaction, block_run &run)
{
	...
	CachedBlock cached(fVolume);
	uint64 block = fVolume->ToBlock(run);
...
status_t
Inode::_GrowStream(Transaction &transaction, uint64 size)
{
	data_stream *data = &Node().data;
	...
	uint64 blocksRequested = (bytes + fVolume->BlockSize() - 1)
		>> fVolume->BlockShift();
	if (blocksRequested > fVolume->FreeBlocks())
		return B_DEVICE_FULL;

	uint64 blocksNeeded = blocksRequested;
	...
	while (blocksNeeded > 0) {
		...
		if (data->Size() <= data->MaxDoubleIndirectRange()
			...
			uint64 start = data->MaxDoubleIndirectRange() - data->MaxIndirectRange();
...
Inode::_FreeStaticStreamArray(Transaction &transaction, int32 level,
	block_run run, uint64 size, uint64 offset, uint64 &max)
{
	...
	offset += (uint64)index * indirectSize;
	...
	uint32 arrayLength, uint64 size, uint64 &offset, uint64 &max)
{
	...
	for (; i < arrayLength; i++, offset = newOffset) {
		...
		newOffset += (uint64)array[i].Length() << fVolume->BlockShift();
		if (newOffset <= size)
			continue;
		...
		if (newOffset > size && offset < size) {
			...
			max = HOST_ENDIAN_TO_BFS_INT64(offset + ((uint64)array[i].Length()
				<< fVolume->BlockShift()));
		} else {
			// free the whole block_run
			array[i].SetTo(0, 0, 0);

			if ((uint64)BFS_ENDIAN_TO_HOST_INT64(max) > offset)
...
status_t
Inode::_ShrinkStream(Transaction &transaction, uint64 size)
{
	data_stream *data = &Node().data;
	status_t status;

	if (data->MaxDoubleIndirectRange() > size) {
		uint64 *maxDoubleIndirect = &data->max_double_indirect_range;
			// gcc 4 work-around: "error: cannot bind packed field
			// 'data->data_stream::max_double_indirect_range' to 'uint64&'"
	...
	if (data->MaxIndirectRange() > size) {
		CachedBlock cached(fVolume);
		uint64 block = fVolume->ToBlock(data->indirect);
		uint64 offset = data->MaxDirectRange();
		...
		for (int32 i = 0; i < data->indirect.Length(); i++) {
			...
			uint64 *maxIndirect = &data->max_indirect_range;
				// gcc 4 work-around: "error: cannot bind packed field
				// 'data->data_stream::max_indirect_range' to 'uint64&'"
	...
	if (data->MaxDirectRange() > size) {
		uint64 offset = 0;
		uint64 *maxDirect = &data->max_direct_range;
			// gcc 4 work-around: "error: cannot bind packed field
			// 'data->data_stream::max_direct_range' to 'uint64&'"
...
status_t
Inode::SetFileSize(Transaction &transaction, uint64 size)
{
	if (size < 0)
		return B_BAD_VALUE;
...
status_t
Inode::Append(Transaction &transaction, uint64 bytes)
{
...
Inode::NeedsTrimming()
{
	...
	uint64 roundedSize = (Size() + fVolume->BlockSize() - 1)
		& ~(fVolume->BlockSize() - 1);
...
Inode::Sync()
{
	...
	uint64 indirectBlock = fVolume->ToBlock(data->double_indirect);
