...
class Term {
	public:
		...
		virtual status_t Match(Inode *inode,const char *attribute = NULL,int32 type = 0,
				const uint8 *key = NULL,uint64 size = 0) = 0;
...
class Equation : public Term {
	public:
		...
		virtual status_t Match(Inode *inode, const char *attribute = NULL, int32 type = 0,
						const uint8 *key = NULL, uint64 size = 0);
...
status_t
Equation::Match(Inode *inode, const char *attributeName, int32 type, const uint8 *key, uint64 size)
{
	...
	*iterator = new TreeIterator(tree);
	...
	if ((fOp == OP_EQUAL || fOp == OP_GREATER_THAN || fOp == OP_GREATER_THAN_OR_EQUAL
		...
		if (fIsSpecialTime) {
			...
			uint64 value = fValue.Int64 << INODE_TIME_SHIFT;
...
status_t
Operator::Match(Inode *inode, const char *attribute, int32 type, const uint8 *key, uint64 size)
{
	if (fOp == OP_AND) {
