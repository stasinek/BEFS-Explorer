...
CachedNode::SetTo(uint64 offset, bool check)
{
	if (fTree == NULL || fTree->fStream == NULL) {
...
bplustree_node *
CachedNode::SetToWritable(Transaction &transaction, uint64 offset, bool check)
{
	if (fTree == NULL || fTree->fStream == NULL) {
		REPORT_ERROR(B_BAD_VALUE);
		return NULL;
...
bplustree_node *
CachedNode::InternalSetTo(Transaction *transaction, uint64 offset)
{
	fNode = NULL;
	fOffset = offset;
...
status_t
CachedNode::Free(Transaction &transaction, uint64 offset)
{
	...
	uint64 lastOffset = header->MaximumSize() - fTree->fNodeSize;
	...
	uint64 *_offset)
{
	...
	uint64 offset = header->MaximumSize();
...
void
BPlusTree::_UpdateIterators(uint64 offset, uint64 nextOffset, uint16 keyIndex,
...
BPlusTree::_FindKey(const bplustree_node *node, const uint8 *key,
	uint16 keyLength, uint16 *_index, uint64 *_next)
{
#ifdef DEBUG
	...
	for (int16 first = 0, last = node->NumKeys() - 1; first <= last;) {
		...
		if (searchKey + searchLength + sizeof(uint64) + sizeof(uint16)
...
BPlusTree::_FindFreeDuplicateFragment(Transaction &transaction,
	...
	uint64 *_offset, bplustree_node **_fragment, uint32 *_index)
{
	uint64 *values = node->Values();
	for (int32 i = 0; i < node->NumKeys(); i++) {
		uint64 value = BFS_ENDIAN_TO_HOST_INT64(values[i]);
...
BPlusTree::_InsertDuplicate(Transaction &transaction, CachedNode &cached,
	const bplustree_node *node, uint16 index, uint64 value)
{
	CachedNode cachedDuplicate(this);
	uint64 *values = node->Values();
	uint64 oldValue = BFS_ENDIAN_TO_HOST_INT64(values[index]);
	status_t status;
	uint64 offset;
			...
			if (array->count < NUM_FRAGMENT_VALUES) {
				...
				if (duplicate->FragmentsUsed(fNodeSize) < 2) {
					...
					memmove(duplicate->DuplicateArray(), array,
						(NUM_FRAGMENT_VALUES + 1) * sizeof(uint64));
					...
					memcpy(&newDuplicate->all_key_count, &array->values[0],
						array->count * sizeof(uint64));
					memset(array, 0, (NUM_FRAGMENT_VALUES + 1) * sizeof(uint64));
...
*/
status_t
BPlusTree::_SplitNode(bplustree_node *node, uint64 nodeOffset,
	bplustree_node *other, uint64 otherOffset, uint16 *_keyIndex, uint8 *key,
	uint16 *_keyLength, uint64 *_value)
{
	if (*_keyIndex > node->NumKeys() + 1)
		return B_BAD_VALUE;

	uint16 *inKeyLengths = node->KeyLengths();
	uint64 *inKeyValues = node->Values();
	...
	for (in = out = 0; in < node->NumKeys() + 1;) {
		...
		if (round_up(sizeof(bplustree_node) + bytesBefore + bytesAfter + bytes)
				+ out * (sizeof(uint16) + sizeof(uint64)) >= size) {
	...
	uint64 *outKeyValues = other->Values();
	...
	if (bytesBefore) {
		...
		memcpy(outKeyValues, inKeyValues, keys * sizeof(uint64));
	}
	if (bytes) {
	...
	if (node->OverflowLink() != BPLUSTREE_NULL) {
		if (in == keyIndex) {
			...
			if (droppedKey + newLength + sizeof(uint64) + sizeof(uint16)
					> (uint8 *)node + fNodeSize
	...
	if (bytesBefore)
		memmove(outKeyValues, inKeyValues + skip, keys * sizeof(uint64));
	if (bytesAfter) {
	...
	uint64 value)
{
	...
	CachedNode cachedDuplicate(this);
	uint64 duplicateOffset = bplustree_node::FragmentOffset(oldValue);
	bplustree_node *duplicate = cachedDuplicate.SetToWritable(transaction,
	...
	while (true) {
		uint64 left = duplicate->LeftLink();
		uint64 right = duplicate->RightLink();
		...
		if ((isLast && array->count == 1) || array->count == 0) {
			...
			if (_FindFreeDuplicateFragment(transaction, node, cachedOther,
				...
				memcpy(target, array, (NUM_FRAGMENT_VALUES + 1) * sizeof(uint64));

				cachedDuplicate.Free(transaction, duplicateOffset);
				duplicateOffset = offset;
			} else {
				// convert node
				memmove(duplicate, array, (NUM_FRAGMENT_VALUES + 1) * sizeof(uint64));
				memset((uint64 *)duplicate + NUM_FRAGMENT_VALUES + 1, 0,
					fNodeSize - (NUM_FRAGMENT_VALUES + 1) * sizeof(uint64));
			}
...
BPlusTree::_RemoveKey(bplustree_node *node, uint16 index)
{
	...
	if (key + length + sizeof(uint64) + sizeof(uint16) > (uint8 *)node + fNodeSize
	...
	// move values
	if (index > 0)
		memmove(newValues, values, index * sizeof(uint64));
	if (node->NumKeys() > index)
		memmove(newValues + index, values + index + 1, (node->NumKeys() - index) * sizeof(uint64));
}
	...
	uint64 value)
{
	...
	while (stack.Pop(&nodeAndKey)
		...
		if (node->IsLeaf()) {
			...
			uint64 next = node->RightLink() == BPLUSTREE_NULL
	...
	uint16 keyLength, uint64 value)
{
	...
	uint64 nodeOffset = fHeader->RootNode();
	CachedNode cached(this);
	const bplustree_node *node;
...
*/
status_t
BPlusTree::Find(const uint8 *key, uint16 keyLength, uint64 *_value)
{
	if (keyLength < BPLUSTREE_MIN_KEY_LENGTH
		|| keyLength > BPLUSTREE_MAX_KEY_LENGTH
		|| key == NULL)
...
TreeIterator::Goto(int8 to)
{
	...
	uint64 nodeOffset = fTree->fHeader->RootNode();
...
TreeIterator::Traverse(int8 direction, void *key, uint16 *keyLength,
	uint16 maxLength, uint64 *value, uint16 *duplicate)
{
...
//	printf("TreeIterator::Traverse 8\n");
	uint64 savedNodeOffset = fCurrentNodeOffset;
...
//	printf("TreeIterator::Traverse 11 - fCurrentKey=%i\n", fCurrentKey);
	...
	if (keyStart + length + sizeof(uint64) + sizeof(uint16) > (uint8 *)node + fTree->fNodeSize
		|| length > BPLUSTREE_MAX_KEY_LENGTH) {
...
//	printf("TreeIterator::Traverse 13\n");
	uint64 offset = BFS_ENDIAN_TO_HOST_INT64(node->Values()[fCurrentKey]);
...
void 
TreeIterator::Update(uint64 offset, uint64 nextOffset, uint16 keyIndex, uint16 splitAt, int8 change)
{
...
uint8
bplustree_node::CountDuplicates(uint64 offset, bool isFragment) const
{
...
uint64
bplustree_node::DuplicateAt(uint64 offset, bool isFragment, int8 index) const
{
	uint32 start;
	if (isFragment)
	...
	return ((uint64 *)this)[start + 1 + index];
}
...
bplustree_node::CheckIntegrity(uint32 nodeSize) const
{
	...
	for (int32 i = 0; i < NumKeys(); i++) {
		...
		if (key + length + sizeof(uint64) + sizeof(uint16) > (uint8 *)this + nodeSize
			|| length > BPLUSTREE_MAX_KEY_LENGTH) {
