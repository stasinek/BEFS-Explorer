...
struct block_cache {
	...
	block_cache(HANDLE fd, uint64 numBlocks, size_t blockSize, bool readOnly);
	~block_cache();
	...
	cached_block *NewBlock(uint64 blockNumber);
...
struct file_map {
	...
	status_t Add(file_io_vec *vecs, size_t vecCount, uint64 &lastOffset);
	void Free();
...
//not implemented:
status_t write_to_cache(file_cache_ref *ref, uint64 offset, size_t size, addr_t buffer, size_t bufferSize);
...
struct page_hash_key {
	page_hash_key() {}
	page_hash_key(HANDLE fd, vnode_id id, uint64 offset)
		: deviceFD(fd),
		  nodeID(id),
		  offset(offset)
	{
...
status_t	block_cache_make_writable(void *_cache, uint64 blockNumber, int32_t transaction);
void *		block_cache_get_writable_etc(void *_cache,uint64 blockNumber, uint64 base,uint64 length, int32_t transaction);
void *		block_cache_get_writable(void *_cache,uint64 blockNumber, int32_t transaction);
void *		block_cache_get_empty(void *_cache,uint64 blockNumber, int32_t transaction);
const void * block_cache_get_etc(void *_cache, uint64 blockNumber, uint64 base, uint64 length/*, uint64 offset*/);
const void * block_cache_get(void *_cache,	uint64 blockNumber/*, uint64 offset*/);
status_t	block_cache_set_dirty(void *_cache, uint64 blockNumber, bool isDirty,int32_t transaction);
void		block_cache_put(void *_cache,uint64 blockNumber);

// File cache stuff

void *		file_cache_create(mount_id mountID,vnode_id vnodeID, uint64 size, HANDLE fd, Volume * volume);
...
//status_t	file_cache_invalidate_file_map(void *_cacheRef,uint64 offset, uint64 size);
