 ...
 */
#ifndef BFS_JOURNAL_H
#define BFS_JOURNAL_H
...
class Journal {
	public:
		...
		status_t LogBlocks(uint64 blockNumber, const uint8 *buffer, size_t numBlocks);
...
class Transaction {
	public:
		Transaction(Volume *volume, uint64 refBlock)
			:
			fJournal(NULL)
		{
		...
		status_t Start(Volume *volume, uint64 refBlock);
		...
		status_t WriteBlocks(uint64 blockNumber, const uint8 *buffer, size_t numBlocks = 1)
		{
			if (fJournal == NULL)
				return B_NO_INIT;
