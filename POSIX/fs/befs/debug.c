...
void befs_dump_inode (const befs_super_block* sb, befs_inode * befs_inode)
{
	#ifdef CONFIG_BEFS_DEBUG
	...
	befs_debug(sb, "  magic1 %08x", fs32_to_cpu(sb, befs_inode->magic1));
	
	tmp_run = fsrun_to_cpu(sb, befs_inode->inode_num);
	befs_debug(sb, "  inode_num %u, %hu, %hu", 
			tmp_run.allocation_group,
			tmp_run.start, 
			tmp_run.len); 
	
	befs_debug(sb, "  uid %u", fs32_to_cpu(sb, befs_inode->uid));
	befs_debug(sb, "  gid %u", fs32_to_cpu(sb, befs_inode->gid));
	befs_debug(sb, "  mode %08x", fs32_to_cpu(sb, befs_inode->mode));
	befs_debug(sb, "  flags %08x", fs32_to_cpu(sb, befs_inode->flags));
	befs_debug(sb, "  create_time %Lu", 
			fs64_to_cpu(sb, befs_inode->create_time));
	befs_debug(sb, "  last_modified_time %Lu", 
			fs64_to_cpu(sb, befs_inode->last_modified_time));
	
	tmp_run = fsrun_to_cpu(sb, befs_inode->parent);
	befs_debug(sb, "  parent [%u, %hu, %hu]", 
		tmp_run.allocation_group,
		tmp_run.start, 
		tmp_run.len);
	
	tmp_run = fsrun_to_cpu(sb, befs_inode->attributes);
	befs_debug(sb, "  attributes [%u, %hu, %hu]",
		tmp_run.allocation_group,
		tmp_run.start,
		tmp_run.len);
	
	befs_debug(sb, "  type %08x", fs32_to_cpu(sb, befs_inode->type));
	befs_debug(sb, "  inode_size %u", fs32_to_cpu(sb, befs_inode->inode_size));

	if (S_ISLNK(befs_inode->mode)) {
		befs_debug(sb, "  Symbolic link [%s]", befs_inode->data.symlink);
	} else {
		int i;

		for (i = 0; i < BEFS_NUM_DIRECT_BLOCKS; i++) {
			tmp_run = fsrun_to_cpu(sb, befs_inode->data.datastream.direct[i]);
			befs_debug(sb, "  direct %d [%u, %hu, %hu]",
				i,
				tmp_run.allocation_group,
				tmp_run.start,
				tmp_run.len);
		...
		befs_debug(sb, "  max_direct_range %Lu",
			fs64_to_cpu(sb, befs_inode->data.datastream.max_direct_range));
		
		tmp_run = fsrun_to_cpu(sb, befs_inode->data.datastream.indirect);
		...
		befs_debug(sb, "  max_indirect_range %Lu",
			fs64_to_cpu(sb, befs_inode->data.datastream.max_indirect_range));
		
		tmp_run = fsrun_to_cpu(sb, befs_inode->data.datastream.double_indirect);
		...
		befs_debug(sb, "  max_double_indirect_range %Lu",
			fs64_to_cpu(sb, befs_inode->data.datastream.max_double_indirect_range));
		
		befs_debug(sb, "  size %Lu",
			fs64_to_cpu(sb, befs_inode->data.datastream.size));
	}
...
void befs_dump_super_block (const befs_super_block* sb, befs_super_block* sup)
{
	#ifdef CONFIG_BEFS_DEBUG
...
void befs_dump_small_data (const befs_super_block* sb, befs_small_data * sd)
{
}



void befs_dump_run (const befs_super_block* sb, befs_block_run run)
{
	#ifdef CONFIG_BEFS_DEBUG
...
void befs_dump_index_entry (const befs_super_block* sb, 
		befs_btree_super *super)
{
	#ifdef CONFIG_BEFS_DEBUG
...
void befs_dump_index_node (const befs_super_block* sb, 
