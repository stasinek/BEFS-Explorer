...
static int befs_find_brun_direct(befs_super_block* sb, 
		befs_data_stream* data, befs_blocknr_t blockno, befs_block_run* run);

static int befs_find_brun_indirect(befs_super_block* sb, 
		befs_data_stream* data, befs_blocknr_t blockno, befs_block_run* run);

static int befs_find_brun_dblindirect(befs_super_block* sb, 
		befs_data_stream* data, befs_blocknr_t blockno, befs_block_run* run);
 ...
 */
struct buffer_head*
befs_read_datastream(befs_super_block* sb, befs_data_stream* ds,
			befs_off_t pos, uint* off)
{
	...
	if(off) *off = pos - (block << BEFS_SB(sb)->block_shift);
	...
	befs_blocknr_t fblock, befs_block_run* run)
{
	...
	if (pos < data->max_direct_range) {
		...
		befs_error(sb, "befs_fblock2brun() was asked to find block %lu, "
 ...
 */
size_t
befs_read_lsymlink(befs_super_block* sb, befs_data_stream* ds, void* buff,
			befs_off_t len)
{
	...
	befs_debug(sb, "<--- befs_read_lsymlink() read %u bytes",
			 bytes_read);	
	return bytes_read;
}
 ...
 * befs_inode occupies on the filesystem, counting both regular file
...
befs_blocknr_t befs_count_blocks(befs_super_block* sb, befs_data_stream* ds)
{
	...
	metablocks = 1; /* Start with 1 block for befs_inode */
	...
	*/
	if (ds->size > ds->max_indirect_range && ds->max_indirect_range != 0) {
		...
		dbl_bytes = ds->max_double_indirect_range - ds->max_indirect_range;
		dbl_bruns = dbl_bytes / (befs_sb->block_size * BEFS_DBLINDIR_BRUN_LEN);
...
/*
	...
	2001-11-15 Will Dyson
*/
static int befs_find_brun_direct(befs_super_block* sb, befs_data_stream* data,
	...
	befs_data_stream* data, befs_blocknr_t blockno, befs_block_run* run)
{
	...
	for (i=0; i < indirect.len; i++) {
		...
		if (indirblock == NULL) {
			befs_debug(sb, "---> befs_find_brun_indirect() failed to "
		...
		for (j=0; j < arraylen; ++j) {
			...
			if (search_blk >= sum && search_blk < sum + len) {
				...
				run->start = fs16_to_cpu(sb, array[j].start) + offset;
				run->len = fs16_to_cpu(sb, array[j].len) - offset;
				
				brelse(indirblock);
				befs_debug(sb, "<--- befs_find_brun_indirect() found "
...
/*
	...
	befs_inode addresses in the double-indirect block will point us
	to the indirect block that contains the mapping for the data,
	then we calculate which of the befs_inode addresses in that 
	indirect block maps the data block we are after.
	
	Oh, and once we've done that, we actually read in the blocks 
	that contain the befs_inode addresses we calculated above. Even 
	...
	befs_data_stream* data,	befs_blocknr_t blockno, befs_block_run* run)
{
	...
	dbl_indir_block = befs_bread(sb, 
			iaddr2blockno(sb, &data->double_indirect) + dbl_which_block);
	...
	indir_block = befs_bread(sb, iaddr2blockno(sb, &indir_run) + which_block);
	if (indir_block == NULL) {
		befs_error(sb, "befs_read_brun_dblindirect() couldn't read the "
