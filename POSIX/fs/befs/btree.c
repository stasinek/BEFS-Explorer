 ...
 * duplicate keys and non-string keytypes (int32_t, int64_t, float, double).
 */
...
/* local functions */
static int befs_btree_seekleaf(befs_super_block* sb, befs_data_stream* ds, 
		befs_btree_super* bt_super, befs_btree_node* this_node, 
		befs_off_t* node_off);

static int befs_bt_read_super(befs_super_block* sb, befs_data_stream* ds, 
		befs_btree_super* sup);

static int befs_bt_read_node(befs_super_block* sb, befs_data_stream* ds, 
...
static int befs_find_key(befs_super_block* sb, befs_btree_node* node, 
		const char* findkey, befs_off_t* value);

static char* befs_bt_get_key(befs_super_block* sb, befs_btree_node* node, 
 ...
 * with @key (usually the disk block number of an befs_inode).
 *
		    ...
		    const char* findkey, befs_off_t* value) 
{
	...
	*value = 0;
	...
	if (eq < 0) *value = fs64_to_cpu(sb, valarray[mid +1]);
	...
	befs_off_t* value)
{
	...
	if (befs_bt_read_super(sb, ds, &bt_super) != BEFS_OK) {
		befs_error(sb, "befs_btree_read() failed to read index superblock");
		goto error;
	}
	...
	befs_debug(sb, "Read [%Lu,%d]: keysize %d",
			node_off, cur_key, keylen);
		...
		befs_off_t* node_off)
{
	...
	while (!befs_leafnode(this_node)) {
		...
		if (befs_bt_read_node(sb, ds, this_node, 
					*node_off) != BEFS_OK) {
			befs_error(sb, "befs_btree_seekleaf() failed to read "
				"node at %Lu", *node_off);
			goto error;
...
#if 0
static int btree_compare_int32(cont void* key1, int keylen1, 
		const void* key2, int keylen2)
{
...
static int btree_compare_int64(cont void* key1, int keylen1, 
		const void* key2, int keylen2)
{
...
static int btree_compare_float(cont void* key1, int keylen1, 
		const void* key2, int keylen2)
{
