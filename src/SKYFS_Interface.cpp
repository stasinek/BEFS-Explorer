...
//static 
status_t skyfs_read_dir(void *_ns, void *_node, void *_cookie, struct dirent_t *Dirent,
	size_t bufferSize, uint32_t *_num)
{
    ...
    Dirent->d_reclen = sizeof(struct dirent_t) + length;
...
status_t skyfs_open(void *_fs, void *_node, int openMode, void **_cookie)
{
    ...
    status_t l_status = inode->CheckPermissions(openModeToAccess(openMode)
		| (openMode & O_TRUNC ? W_OK : 0));
    if (l_status < B_OK)
        RETURN_ERROR(l_status);
...
//static 
status_t skyfs_read(void *_ns, void *_node, void *_cookie, int64_t pos, void *buffer, int64_t *_length)
{
	...
	return inode->ReadAt(pos, (uint8_t *)buffer, _length);
}
...
status_t skyfs_read_vnode(void *_ns, vnode_id id, void **_node, bool reenter)
{
	...
	if (id < volume->ToBlock(volume->Log()) + volume->Log().Length()
		|| id > volume->NumBlocks()) {
            printf("error: inode at %I64d requested!\n", id);
		return B_ERROR;
	}
	...
	if (node == NULL) {
        FATAL(("could not read inode: %I64d\n", id));
		return B_IO_ERROR;
	}

	status_t status = node->InitCheck(volume);
	if (status < B_OK) {
        FATAL(("inode at %I64d is corrupt!\n", id));
		return status;
	}
...
status_t skyfs_lookup(void *_ns, void *_directory, const char *file, vnode_id *_vnodeID, int *_type)
{
	...
	if ((status = tree->Find((uint8_t *)file, (uint16_t)strlen(file), _vnodeID)) < B_OK) {
...
status_t skyfs_read_attr_dir(void *_ns, void *node, void *_cookie, struct dirent_t *a_dirent,size_t bufferSize, uint32_t *_num)
{
	//FUNCTION();
	AttributeIterator *iterator = (AttributeIterator *)_cookie;
	...
	uint32_t type;
	size_t length;
    status_t status = iterator->GetNext(a_dirent->d_name, &length, &type, &a_dirent->d_ino);
    ...
    a_dirent->d_dev = volume->ID();
    a_dirent->d_reclen = sizeof(struct dirent_t) + length;
...
status_t skyfs_read_attr(void* _fs, fs_vnode _file, attr_cookie* _cookie, int64_t pos,void *buffer, int64_t *_length)
{
	//FUNCTION();
	...
	return attribute.Read(cookie, pos, (uint8_t *)buffer, _length);
}
