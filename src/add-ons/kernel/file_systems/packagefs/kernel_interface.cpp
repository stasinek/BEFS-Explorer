...
static const uint32_t kOptimalIOSize = 64 * 1024;
...
static status_t
packagefs_mount(fs_volume* fsVolume, const char* device, uint32_t flags,
...
packagefs_ioctl(fs_volume* fsVolume, fs_vnode* fsNode, void* cookie,
	uint32_t operation, void* buffer, size_t size)
{
	Volume* volume = (Volume*)fsVolume->private_volume;
	Node* node = (Node*)fsNode->private_node;
...
packagefs_read_dir(fs_volume* fsVolume, fs_vnode* fsNode, void* _cookie,
	struct dirent* buffer, size_t bufferSize, uint32_t* _count)
{
	Volume* volume = (Volume*)fsVolume->private_volume;
	Node* node = (Node*)fsNode->private_node;
	DirectoryCookie* cookie = (DirectoryCookie*)_cookie;
...
status_t
packagefs_create_index(fs_volume* fsVolume, const char* name, uint32_t type,
	uint32_t flags)
{
...
status_t
packagefs_open_query(fs_volume* fsVolume, const char* queryString, uint32_t flags,
