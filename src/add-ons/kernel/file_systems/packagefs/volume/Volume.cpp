...
static const uint32_t kAllStatFields = B_STAT_MODE | B_STAT_UID | B_STAT_GID
...
public:
	...
	status_t Init(const void* userRequest, size_t requestSize)
	{
		...
		uint32_t itemCount = fRequest->itemCount;
	...
	PackageFSActivationChangeItem* ItemAt(uint32_t index) const
	{
...
status_t
Volume::IOCtl(Node* node, uint32_t operation, void* buffer, size_t size)
{
	switch (operation) {
		...
		case PACKAGE_FS_OPERATION_GET_PACKAGE_INFOS:
		{
			...
			uint32_t packageCount = fPackages.CountElements();
...
void
Volume::UpdateLiveQueries(Node* node, const char* attribute, int32_t type,
...
void
Volume::PackageLinkNodeChanged(Node* node, uint32_t statFields,
...
{
	uint32_t itemCount = request.CountItems();
	if (itemCount == 0)
		return B_OK;

	// first check the request
	int32_t newPackageCount = 0;
	int32_t oldPackageCount = 0;
	{
...
INFORM("package \"%s\" activated\n", package->FileName().Data());
	...
	if (error != B_OK) {
		for (int32_t i = newPackageIndex - 1; i >= 0; i--) {
			Package* package = newPackageReferences[i];
			_RemovePackageContent(package, NULL, true);
			_RemovePackage(package);
		}

		for (int32_t i = oldPackageCount - 1; i >= 0; i--) {
...
void
Volume::_NotifyNodeChanged(Node* node, uint32_t statFields,
	const OldNodeAttributes& oldAttributes)
{
	Node* key = node;
