...
void
SizeIndex::NodeChanged(Node* node, uint32_t statFields,
	const OldNodeAttributes& oldAttributes)
{
	...
	fVolume->UpdateLiveQueries(node, Name(), Type(),
		(const uint8_t*)&oldSize, sizeof(oldSize), (const uint8_t*)&newSize,
		sizeof(newSize));
}
...
void
SizeIndex::Iterator::NodeChanged(Node* node, uint32_t statFields,
	const OldNodeAttributes& oldAttributes)
{
