...
void
LastModifiedIndex::NodeChanged(Node* node, uint32_t statFields,
	const OldNodeAttributes& oldAttributes)
{
	...
	fVolume->UpdateLiveQueries(node, Name(), Type(),
		(const uint8_t*)&oldLastModified, sizeof(oldLastModified),
		(const uint8_t*)&newLastModified, sizeof(newLastModified));
}
...
void
LastModifiedIndex::Iterator::NodeChanged(Node* node, uint32_t statFields,
	const OldNodeAttributes& oldAttributes)
{
