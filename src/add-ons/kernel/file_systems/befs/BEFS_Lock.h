...
class RecursiveLock {
    public:
        ...
        int32_t OwnerCount() const { return fOwnerCount; }
...
// the semaphore count exceeds the int32_t bounds
//---------------------------------------------------------------------------

//---------------------------------------------------------------------------
// Timeouts: It may be a good idea to have timeouts for the WriteLocked class,
...
// semaphores (since every open BEFS inode has such a lock).
//---------------------------------------------------------------------------
...
class ReadWriteLock {
    public:
        ...
        status_t LockWrite()
        {
            ...
            int32_t readers = atomic_add(&fCount, -MAX_READERS);
...
class SimpleLock {
    public:
        ...
        status_t Lock(bigtime_t time = 500)
        {
            ...
            //TODO: verify
            //int32_t thisThread = find_thread(NULL);
