...
class AllocationBlock : public CachedBlock {
	public:
		...
		void Allocate(uint16_t start, uint16_t numBlocks);
		void Free(uint16_t start, uint16_t numBlocks);
		inline bool IsUsed(uint16_t block);

		status_t SetTo(AllocationGroup &group, uint16_t block);
		status_t SetToWritable(Transaction &transaction, AllocationGroup &group, uint16_t block);

		uint32_t NumBlockBits() const { return fNumBits; }
		uint32_t &Block(int32_t index) { return ((uint32_t *)fBlock)[index]; }
...
class AllocationGroup {
	public:
		...
		void AddFreeRange(int32_t start, int32_t blocks);
		bool IsFull() const { return fFreeBits == 0; }

		status_t Allocate(Transaction &transaction, uint16_t start, int32_t length);
		status_t Free(Transaction &transaction, uint16_t start, int32_t length);

		uint32_t NumBits() const { return fNumBits; }
		uint32_t NumBlocks() const { return fNumBlocks; }
		int32_t Start() const { return fStart; }
	...
	private:
		...
		uint32_t	fNumBlocks;
		int32_t	fStart;
		int32_t	fFirstFree, fLargest, fLargestFirst;
...
status_t
AllocationBlock::SetTo(AllocationGroup &group, uint16_t block)
{
	// 8 blocks per byte
...
status_t
AllocationBlock::SetToWritable(Transaction &transaction, AllocationGroup &group, uint16_t block)
{
	// 8 blocks per byte
	fNumBits = fVolume->BlockSize() << 3;
...
bool
AllocationBlock::IsUsed(uint16_t block)
{
	if (block > fNumBits)
		return true;

	// the block bitmap is accessed in 32-bit blocks
    return Block(block >> 5) & HOST_ENDIAN_TO_BEFS_INT32(1UL << (block % 32));
}
...
AllocationBlock::Allocate(uint16_t start, uint16_t numBlocks)
{
	...
	int32_t block = start >> 5;

	while (numBlocks > 0) {
		uint32_t mask = 0;
		for (int32_t i = start % 32; i < 32 && numBlocks; i++, numBlocks--)
			mask |= 1UL << (i % 32);

#ifdef DEBUG
		// check for already set blocks
        if (HOST_ENDIAN_TO_BEFS_INT32(mask) & ((uint32_t *)fBlock)[block]) {
			FATAL(("AllocationBlock::Allocate(): some blocks are already allocated, start = %u, numBlocks = %u\n", start, numBlocks));
			DEBUGGER(("blocks already set!"));
		}
#endif
        Block(block++) |= HOST_ENDIAN_TO_BEFS_INT32(mask);
		start = 0;
	}
}
...
void AllocationGroup::AddFreeRange(int32_t start, int32_t blocks)
{

	//D(if (blocks > 512)
    //	PRINT(("range of %I64d blocks starting at %I64d\n",blocks,start)));
...
status_t
AllocationGroup::Allocate(Transaction &transaction, uint16_t start, int32_t length)
{
	if (start > fNumBits)
		return B_ERROR;
	...
	uint32_t bitsPerBlock = volume->BlockSize() << 3;
	uint32_t block = start / bitsPerBlock;
...
status_t
AllocationGroup::Free(Transaction &transaction, uint16_t start, int32_t length)
{
	if (start > fNumBits)
		return B_ERROR;
...
BlockAllocator::InitializeAndClearBitmap(Transaction &transaction)
{
	...
	uint32_t blocks = fBlocksPerGroup;
	...
	for (int32_t i = 0; i < fNumGroups; i++) {
	...
	uint32_t reservedBlocks = fVolume->Log().Start() + fVolume->Log().Length();

	if (fGroups[0].Allocate(transaction, 0, reservedBlocks) < B_OK) {
		FATAL(("could not allocate reserved space for block bitmap/log!\n"));
		return B_ERROR;
	}
    fVolume->SuperBlock().used_blocks = HOST_ENDIAN_TO_BEFS_INT64(reservedBlocks);
...
BlockAllocator::_Initialize(BlockAllocator *allocator)
{
	...
	uint32_t blocks = allocator->fBlocksPerGroup;
    ...
    int64_t offset = 1;
	uint32_t bitsPerGroup = 8 * (blocks << blockShift);
	int32_t numGroups = allocator->fNumGroups;
	...
	for (int32_t i = 0; i < numGroups; i++) {
		...
		int32_t start = -1, range = 0;
		int32_t numBits = groups[i].fNumBits, bit = 0;
		int32_t count = (numBits + 31) / 32;

		for (int32_t k = 0; k < count; k++) {
			for (int32_t j = 0; j < 32 && bit < numBits; j++, bit++) {
				if (buffer[k] & (1UL << j)) {
					// block is in use
					if (range > 0) {
	...
	uint32_t reservedBlocks = volume->Log().Start() + volume->Log().Length();
    ...
    int64_t usedBlocks = volume->NumBlocks() - freeBlocks;
	if (volume->UsedBlocks() != usedBlocks) {
		// If the disk in a dirty state at mount time, it's
		// normal that the values don't match
        INFORM(("volume reports %I64d used blocks, correct is %I64d\n", volume->UsedBlocks(), usedBlocks));
        volume->SuperBlock().used_blocks = HOST_ENDIAN_TO_BEFS_INT64(usedBlocks);
	}
...
status_t
BlockAllocator::AllocateBlocks(Transaction &transaction, int32_t group, uint16_t start,
	uint16_t maximum, uint16_t minimum, block_run &run)
{
	if (maximum == 0)
		return B_BAD_VALUE;

    FUNCTION_START(("group = %I64d, start = %u, maximum = %u, minimum = %u\n", group, start, maximum, minimum));
	...
	uint16_t numBlocks = maximum;

	for (int32_t i = 0; i < fNumGroups * 2; i++, group++, start = 0) {
		...
		uint32_t block = start / (fVolume->BlockSize() << 3);
		int32_t range = 0, rangeStart = 0;
		...
		for (; block < fGroups[group].NumBlocks(); block++) {
			...
			for (uint32_t bit = start % cached.NumBlockBits();
			...
			if (range >= numBlocks) {
                ...
                run.start = HOST_ENDIAN_TO_BEFS_INT16(rangeStart);
                run.length = HOST_ENDIAN_TO_BEFS_INT16(numBlocks);

				fVolume->SuperBlock().used_blocks =
                    HOST_ENDIAN_TO_BEFS_INT64(fVolume->UsedBlocks() + numBlocks);
	...
	mode_t type, block_run &run)
{
	...
	uint16_t group = parent->AllocationGroup();
...
status_t
BlockAllocator::Allocate(Transaction &transaction, Inode *inode, int64_t numBlocks,
	block_run &run, uint16_t minimum)
{
	if (numBlocks <= 0)
		return B_ERROR;
	...
	// since block_run.length is uint16_t, the largest number of blocks that
	...
	// apply some allocation policies here (AllocateBlocks() will break them
	// if necessary)
	uint16_t group = inode->BlockRun().AllocationGroup();
	uint16_t start = 0;
...
BlockAllocator::Free(Transaction &transaction, block_run run)
{
	...
	int32_t group = run.AllocationGroup();
	uint16_t start = run.Start();
	uint16_t length = run.Length();

    FUNCTION_START(("group = %I64d, start = %u, length = %u\n", group, start, length));
	...
	fVolume->SuperBlock().used_blocks =
        HOST_ENDIAN_TO_BEFS_INT64(fVolume->UsedBlocks() - run.Length());
	return B_OK;
}
...
{
	if (control == NULL
        || control->magic != BEFS_IOCTL_CHECK_MAGIC) {
...
BlockAllocator::StartChecking(check_control *control)
{
	...
	for (int32_t block = fVolume->Log().Start() + fVolume->Log().Length(); block-- > 0;) {
...
BlockAllocator::StopChecking(check_control *control)
{
	...
	if (control != NULL && control->status == B_ENTRY_NOT_FOUND) {
		...
		for (uint32_t i = size >> 2; i-- > 0;) {
			uint32_t compare = 1;
			for (int16_t j = 0; j < 32; j++, compare <<= 1) {
        ...
        if (control->flags & BEFS_FIX_BITMAP_ERRORS
            ...
            fVolume->SuperBlock().used_blocks = HOST_ENDIAN_TO_BEFS_INT64(usedBlocks);

			int32_t blocksInBitmap = fNumGroups * fBlocksPerGroup;
			int32_t blockSize = fVolume->BlockSize();

			for (int32_t i = 0; i < blocksInBitmap; i += 512) {
				Transaction transaction(fVolume, 1 + i);

				int32_t blocksToWrite = 512;
				if (blocksToWrite + i > blocksInBitmap)
					blocksToWrite = blocksInBitmap - i;

				status_t status = transaction.WriteBlocks(1 + i,
					(uint8_t *)fCheckBitmap + i * blockSize, blocksToWrite);
				if (status < B_OK) {
	...
	while (true) {
		if (cookie->iterator == NULL) {
			...
			if (vnode.Get(&inode) < B_OK) {
                FATAL(("check: Could not open inode at %I64d\n", fVolume->ToBlock(cookie->current)));
				continue;
			}

			if (!inode->IsContainer()) {
                FATAL(("check: inode at %I64d should have been a directory\n", fVolume->ToBlock(cookie->current)));
				continue;
			}

			BPlusTree *tree;
			if (inode->GetTree(&tree) != B_OK) {
                FATAL(("check: could not open b+tree from inode at %I64d\n", fVolume->ToBlock(cookie->current)));
				continue;
			}
		...
		if (status == B_ENTRY_NOT_FOUND) {
			...
			Inode *inode;
			if (vnode.Get(&inode) < B_OK) {
                FATAL(("Could not open inode ID %I64d!\n", id));
                control->errors |= BEFS_COULD_NOT_OPEN;
			...
			if (inode->IsRegularNode()) {
				...
				if (localName == NULL || strcmp(localName, name)) {
                    control->errors |= BEFS_NAMES_DONT_MATCH;
			...
			if (((cookie->parent_mode & S_ATTR_DIR) != 0 && !inode->IsAttribute())
                ...
                FATAL(("inode at %I64d is of wrong type: %o (parent %o at %I64d)!\n",
                ...
                if (control->flags & BEFS_REMOVE_WRONG_TYPES
                    && control->flags & BEFS_FIX_BITMAP_ERRORS) {
                    ...
                    inode->Node().flags |= HOST_ENDIAN_TO_BEFS_INT32(INODE_DONT_FREE_SPACE);
                ...
                control->errors |= BEFS_WRONG_TYPE;
...
bool
BlockAllocator::_CheckBitmapIsUsedAt(int64_t block) const
{
	size_t size = BitmapSize();
	uint32_t index = block / 32;	// 32bit resolution
	if (index > size / 4)
		return false;

    return BEFS_ENDIAN_TO_HOST_INT32(fCheckBitmap[index]) & (1UL << (block & 0x1f));
}


void
BlockAllocator::_SetCheckBitmapAt(int64_t block)
{
	size_t size = BitmapSize();
	uint32_t index = block / 32;	// 32bit resolution
	if (index > size / 4)
    ...
    fCheckBitmap[index] |= HOST_ENDIAN_TO_BEFS_INT32(1UL << (block & 0x1f));
}
...
BlockAllocator::CheckBlockRun(block_run run, const char *type, check_control *control, bool allocated)
{
	if (run.AllocationGroup() < 0 || run.AllocationGroup() >= fNumGroups
        ...
        control->errors |= BEFS_INVALID_BLOCK_RUN;
		return B_OK;
	}

	uint32_t bitsPerBlock = fVolume->BlockSize() << 3;
	uint32_t block = run.Start() / bitsPerBlock;
	uint32_t pos = run.Start() % bitsPerBlock;
	int32_t length = 0;
    int64_t firstMissing = -1, firstSet = -1;
    int64_t firstGroupBlock = (int64_t)run.AllocationGroup() << fVolume->AllocationGroupShift();
	...
	for (; block < fBlocksPerGroup && length < run.Length(); block++, pos = 0) {
		...
		while (length < run.Length() && pos < cached.NumBlockBits()) {
			if (cached.IsUsed(pos) != allocated) {
				if (control == NULL) {
                    PRINT(("%s: block_run(%I64d, %u, %u) is only partially allocated (pos = %I64d, length = %I64d)!\n",
						type, run.AllocationGroup(), run.Start(), run.Length(), pos, length));
					return B_BAD_DATA;
				}
				if (firstMissing == -1) {
					firstMissing = firstGroupBlock + pos + block * bitsPerBlock;
                    control->errors |= BEFS_MISSING_BLOCKS;
				}
				control->stats.missing++;
			} else if (firstMissing != -1) {
                PRINT(("%s: block_run(%I64d, %u, %u): blocks %I64d - %I64d are %sallocated!\n",
			...
			if (fCheckBitmap != NULL) {
				...
				uint32_t offset = pos + block * bitsPerBlock;
				if (_CheckBitmapIsUsedAt(firstGroupBlock + offset)) {
					if (firstSet == -1) {
						firstSet = firstGroupBlock + offset;
                        control->errors |= BEFS_BLOCKS_ALREADY_SET;
					}
					control->stats.already_set++;
				} else {
					if (firstSet != -1) {
                        FATAL(("%s: block_run(%d, %u, %u): blocks %I64d - %I64d are already set!\n",
...
BlockAllocator::CheckInode(Inode *inode, check_control *control)
{
	...
	if (data->max_double_indirect_range) {
		...
		int32_t maxIndirectIndex = (data->double_indirect.Length() << fVolume->BlockShift())
									/ sizeof(block_run);

		for (int32_t indirectIndex = 0; indirectIndex < maxIndirectIndex; indirectIndex++) {
			...
			int32_t maxIndex = (indirect.Length() << fVolume->BlockShift()) / sizeof(block_run);

			for (int32_t index = 0; index < maxIndex; ) {
