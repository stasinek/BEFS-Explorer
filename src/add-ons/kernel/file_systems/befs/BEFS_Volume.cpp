...
class DeviceOpener {
	public:
        ...
        void *InitCache(int64_t numBlocks, uint32_t blockSize);
        ...
        status_t GetSize(int64_t *_size, uint32_t *_blockSize = NULL);
...
//	DISK_GEOMETRY *pdg;
	//todo: replace open with CreateFileA!
	//fDevice = open(device, mode);
	//TODO: ignore mode for now
	BOOL bResult; // results flag
	DWORD junk; // discard results
	
	fDevice = CreateFileA(device, // drive to open
	...
	if (fDevice== INVALID_HANDLE_VALUE){
		printf("DeviceOpene::Open: CreateFileA failed\n");
		debug<<"DeviceOpene::Open: CreateFileA failed\n";
		return INVALID_HANDLE_VALUE;
	}else {
		printf("DeviceOpener::Open: CreateFileA success: 0x%p\n",fDevice);
		debug<<"DeviceOpene::Open: CreateFileA success\n";
	}
	...
	*/
	...
	debug<<"DeviceOpener::Open: NT_CreateFile status = "<<Status<<std::endl;
...
//	DISK_GEOMETRY *pdg;
	...
	if (fDevice== INVALID_HANDLE_VALUE){
		printf("DeviceOpene::Open: CreateFileA failed\n");
		return INVALID_HANDLE_VALUE;
	}else printf("DeviceOpener::Open: CreateFileA success: 0x%p\n",fDevice);
...
void *DeviceOpener::InitCache(int64_t numBlocks, uint32_t blockSize)
{
...
status_t DeviceOpener::GetSize(int64_t *_size, uint32_t *_blockSize)
{
	/*
...
*/
	...
	if (_size) {
		...
		*_size = 1 * /*partitionInfo.dwBytesPerSector*/ 512 * partitionInfo.dwNumSectors;
...
{
	//check for a valid SKYFS or BEFS partition
	if ((Magic1() != (int32_t)SUPER_BLOCK_MAGIC1 && Magic1() != (int32_t)BEFS_SUPER_BLOCK_MAGIC1)
		|| (Magic2() != (int32_t)SUPER_BLOCK_MAGIC2 && Magic2() != (int32_t)BEFS_SUPER_BLOCK_MAGIC2)
		|| (Magic3() != (int32_t)SUPER_BLOCK_MAGIC3 && Magic3() != (int32_t)BEFS_SUPER_BLOCK_MAGIC3)
		|| (int32_t)block_size != inode_size
		...
		|| NumBlocks() < 10
		|| AllocationGroups() != divide_roundup((int64_t)NumBlocks(),
			(int32_t)1L << AllocationGroupShift()))
	{
...
void disk_super_block::Initialize(const char *diskName, int64_t numBlocks, uint32_t blockSize)
{
	memset(this, 0, sizeof(disk_super_block));

	magic1 = HOST_ENDIAN_TO_BEFS_INT32(SUPER_BLOCK_MAGIC1);
	magic2 = HOST_ENDIAN_TO_BEFS_INT32(SUPER_BLOCK_MAGIC2);
	magic3 = HOST_ENDIAN_TO_BEFS_INT32(SUPER_BLOCK_MAGIC3);
	fs_byte_order = HOST_ENDIAN_TO_BEFS_INT32(SUPER_BLOCK_FS_LENDIAN);
	flags = HOST_ENDIAN_TO_BEFS_INT32(SUPER_BLOCK_DISK_CLEAN);
//TODO, this should be strlcpy
	strcpy(name, diskName/*, sizeof(name)*/);

	int32_t blockShift = 9;
	while ((1UL << blockShift) < blockSize) {
		blockShift++;
	}

	block_size = inode_size = HOST_ENDIAN_TO_BEFS_INT32(blockSize);
	block_shift = HOST_ENDIAN_TO_BEFS_INT32(blockShift);

	num_blocks = HOST_ENDIAN_TO_BEFS_INT64(numBlocks);
	used_blocks = 0;

	// Get the minimum ag_shift (that's determined by the block size)

	int32_t bitsPerBlock = blockSize << 3;
    int64_t bitmapBlocks = (numBlocks + bitsPerBlock - 1) / bitsPerBlock;
	int32_t blocksPerGroup = 1;
	int32_t groupShift = 13;

	for (int32_t i = 8192; i < bitsPerBlock; i *= 2) {
		groupShift++;
	}
	...
	num_ags = HOST_ENDIAN_TO_BEFS_INT32(numGroups);
	blocks_per_ag = HOST_ENDIAN_TO_BEFS_INT32(blocksPerGroup);
	ag_shift = HOST_ENDIAN_TO_BEFS_INT32(groupShift);
}
...
void dump_inode2(const bfs_inode *inode)
{
	...
	printf("  mode               = %08x\n", (int)inode->mode);
	printf("  flags              = %08x\n", (int)inode->flags);
    printf("  create_time        = %I64d (%I64d)\n", inode->create_time,
		inode->create_time >> INODE_TIME_SHIFT);
    printf("  last_modified_time = %I64d (%I64d)\n", inode->last_modified_time,
...
status_t Volume::Mount(const char *deviceName, int partitionNr, uint32_t flags)
{
...
#if (B_HOST_IS_LENDIAN && defined(BEFS_BIG_ENDIAN_ONLY)) \
	|| (B_HOST_IS_BENDIAN && defined(BEFS_LITTLE_ENDIAN_ONLY))
...
status_t Volume::Mount(int diskNr, int partitionNr, uint32_t flags)
{
	debug<<"Volume::Mount\n";
	// partitionNr x is the x'th skyfs partition, starting at 0 for the first partition
	//skyfsNr = partitionNr;
...
status_t Volume::MountBEFS(const char *deviceName, int partitionNr, uint32_t flags)
{
	// partitionNr x is the x'th skyfs partition, starting at 0 for the first partition
	skyfsNr = partitionNr;
	// ToDo: validate the FS in write mode as well!
...
int Volume::CountSkyfsPartitions(const char* DRIVE,bool skyfs){
	...
	*/
	if(hDrive == INVALID_HANDLE_VALUE){
		printf("ScanPartitions: CreateFileA error: %i\n",GetLastError());
		return GetLastError();
	}
	...
	for(i=0; i<4; i++) /// scanning partitions in the physical disk
	{
		...
		switch(PartitionTbl->chType)
		{
			...
			case PART_SKYFS:
				...
				break;
			case PART_BEFS:
				strcpy(szTmpStr, "BEFS");	//skyfs partition
				if (!skyfs){ 
					printf("found bfs partition!\n");
					count++;
				}
...
block_run Volume::ToBlockRun(int64_t block) const
{
	block_run run;
	run.allocation_group = HOST_ENDIAN_TO_BEFS_INT32(block >> AllocationGroupShift());
	run.start = HOST_ENDIAN_TO_BEFS_INT16(block & ((1 << AllocationGroupShift()) - 1));
	run.length = HOST_ENDIAN_TO_BEFS_INT16(1);
	return run;
}
...
void Volume::UpdateLiveQueries(Inode *inode, const char *attribute, int32_t type, const uint8_t *oldKey,
	size_t oldLength, const uint8_t *newKey, size_t newLength)
{
...
status_t Volume::Identify(HANDLE fd, disk_super_block *superBlock)
{
	...
	printf("  name           = %s\n", superBlock->name);
    printf("  magic1         = %#08x (%s) %s\n", (int)superBlock->magic1, get_tupel2(superBlock->magic1), ((superBlock->magic1 == SUPER_BLOCK_MAGIC1) || (superBlock->magic1 == (int)BEFS_SUPER_BLOCK_MAGIC1)? "valid" : "INVALID"));
    ...
    printf("  magic2         = %#08x (%s) %s\n", (int)superBlock->magic2, get_tupel2(superBlock->magic2), ((superBlock->magic2 == (int)SUPER_BLOCK_MAGIC2) || (superBlock->magic2 == (int)BEFS_SUPER_BLOCK_MAGIC2) ? "valid" : "INVALID"));
	printf("  blocks_per_ag  = %u\n", (unsigned)superBlock->blocks_per_ag);
    printf("  ag_shift       = %u (%I64d bytes)\n", (unsigned)superBlock->ag_shift, 1L << superBlock->ag_shift);
    ...
    printf("  log_end        = %lu\n", superBlock->log_end);
    printf("  magic3         = %#08x (%s) %s\n", (int)superBlock->magic3, get_tupel2(superBlock->magic3), ((superBlock->magic3 == SUPER_BLOCK_MAGIC3) || (superBlock->magic3 == (int)BEFS_SUPER_BLOCK_MAGIC3) ? "valid" : "INVALID"));
    ...
    debug<<"  magic1         = "<<(int)superBlock->magic1<<" "<<get_tupel2(superBlock->magic1)<<"  "<< ((superBlock->magic1 == SUPER_BLOCK_MAGIC1) || (superBlock->magic1 == (int)BEFS_SUPER_BLOCK_MAGIC1)? "valid" : "INVALID")<<std::endl;
    ...
    debug<<"  magic2         = "<<(int)superBlock->magic2<<"  "<<get_tupel2(superBlock->magic2)<<"  "<<((superBlock->magic2 == (int)SUPER_BLOCK_MAGIC2) || (superBlock->magic2 == (int)BEFS_SUPER_BLOCK_MAGIC2) ? "valid" : "INVALID")<<std::endl;
    ...
    debug<<"  magic3         = "<<(int)superBlock->magic3<<"  "<<get_tupel2(superBlock->magic3)<<"  "<<((superBlock->magic3 == SUPER_BLOCK_MAGIC3) || (superBlock->magic3 == (int)BEFS_SUPER_BLOCK_MAGIC3) ? "valid" : "INVALID")<<std::endl;
	//dump_block_run2("  root_dir       = ", superBlock->root_dir);
...
#ifndef BEFS_LITTLE_ENDIAN_ONLY
...
status_t Volume::Initialize(const char *device, const char *name, uint32_t blockSize,
	uint32_t flags)
{
    ...
    int64_t numBlocks = deviceSize / blockSize;
	...
	fSuperBlock.log_blocks.length = HOST_ENDIAN_TO_BEFS_INT16(2048);
		// ToDo: set the log size depending on the disk size
	fSuperBlock.log_start = fSuperBlock.log_end = HOST_ENDIAN_TO_BEFS_INT64(ToBlock(Log()));
