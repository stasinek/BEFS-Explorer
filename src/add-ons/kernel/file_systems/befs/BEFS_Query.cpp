...
class Term {
	public:
		Term(int8_t op) : fOp(op), fParent(NULL) {}
		virtual ~Term() {}

		int8_t		Op() const { return fOp; }

		void		SetParent(Term *parent) { fParent = parent; }
		Term		*Parent() const { return fParent; }

		virtual status_t Match(Inode *inode,const char *attribute = NULL,int32_t type = 0,
				const uint8_t *key = NULL,int64_t size = 0) = 0;
...
class Equation : public Term {
	public:
		...
		virtual status_t Match(Inode *inode, const char *attribute = NULL, int32_t type = 0,
						const uint8_t *key = NULL, int64_t size = 0);
		...
		status_t	GetNextMatching(Volume *volume, TreeIterator *iterator,
                        struct dirent_t *dirent, size_t bufferSize);
	...
	private:
		...
		bool		CompareTo(const uint8_t *value, uint16_t size);
		uint8_t		*Value() const { return (uint8_t *)&fValue; }
...
int32_t getFirstPatternSymbol(char *string)
{
	...
	for (int32_t index = 0;(c = *string++);index++) {
		if (c == '*' || c == '?' || c == '[')
			return index;
	}
	return -1;
	...
	while (*pattern) {
		...
		switch (*pattern++) {
			...
			case '[':
			{
				...
				uint32_t c = utf8ToUnicode(&string);
				bool matched = false;
				...
				while (pattern[0] != ']') {
					...
					uint32_t first = utf8ToUnicode(&pattern);
					...
					if (first == c) {
						...
						uint32_t last = utf8ToUnicode(&pattern);
...
bool
Equation::CompareTo(const uint8_t *value, uint16_t size)
{
	int32_t compare;
	...
	if (fIsPattern) {
		...
		// the index is a shifted int64_t index, but we have to match
		// against an unshifted value (i.e. the last_modified index)
		int64_t timeValue = *(int64_t *)value >> INODE_TIME_SHIFT;
        compare = compareKeys(fType, &timeValue, sizeof(int64_t), &fValue.Int64, sizeof(int64_t));
	} else
...
Equation::Complement()
{
	...
	int8_t complementOp[] = {OP_UNEQUAL, OP_EQUAL, OP_LESS_THAN_OR_EQUAL,
...
Equation::MatchEmptyString()
{
	...
	if (status == B_OK)
		status = CompareTo((const uint8_t *)"", fSize) ? MATCH_OK : NO_MATCH;
...
status_t
Equation::Match(Inode *inode, const char *attributeName, int32_t type, const uint8_t *key, int64_t size)
{
	...
	if (attributeName != NULL && !strcmp(fAttribute, attributeName)) {
		...
		buffer = (uint8_t *)inode->Name(nodeGetter.Node());
		if (buffer == NULL) {
...
#ifdef BEFS_NATIVE_ENDIAN
		buffer = (uint8_t *)&inode->Node().data.size;
#else
...
#ifdef BEFS_NATIVE_ENDIAN
		buffer = (uint8_t *)&inode->Node().last_modified_time;
#else
...
Equation::CalculateScore(Index &index)
{
	...
	fScore = fScore * ((2048 * 1024) / index.Node()->Size());
}
	...
	*iterator = new TreeIterator(tree);
	...
	if ((fOp == OP_EQUAL || fOp == OP_GREATER_THAN || fOp == OP_GREATER_THAN_OR_EQUAL
		...
		if (fIsSpecialTime) {
            ...
            int64_t value = fValue.Int64 << INODE_TIME_SHIFT;
			status = (*iterator)->Find((uint8_t *)&value, keySize);
    ...
    struct dirent_t *dirent, size_t bufferSize)
{
	while (true) {
		...
		status_t status = iterator->GetNextEntry(&indexValue, &keyLength,
			(uint16_t)sizeof(indexValue), &offset, &duplicate);
		if (status < B_OK)
			return status;

		// only compare against the index entry when this is the correct
		// index for the equation
		if (fHasIndex && duplicate < 2 && !CompareTo((uint8_t *)&indexValue, keyLength)) {
		...
		if ((status = vnode.Get(&inode)) != B_OK) {
			REPORT_ERROR(status);
            FATAL(("could not get inode %I64d in index \"%s\"!\n", offset, fAttribute));
			// try with next
			continue;
		}
		...
		if (status == MATCH_OK) {
			...
			if (inode->GetName(dirent->d_name) < B_OK)
                FATAL(("inode %I64d in query has no name!\n", inode->BlockNumber()));

            dirent->d_reclen = sizeof(struct dirent_t) + strlen(dirent->d_name);
		}
...
Operator::Operator(Term *left, int8_t op, Term *right)
	: Term(op),
	fLeft(left),
	fRight(right)
{
...
status_t
Operator::Match(Inode *inode, const char *attribute, int32_t type, const uint8_t *key, int64_t size)
{
	if (fOp == OP_AND) {
...
Query::Query(Volume *volume, Expression *expression, uint32_t flags)
	:
	fVolume(volume),
...
status_t
Query::GetNextEntry(struct dirent_t *a_dirent, size_t size)
{
	...
	while (true) {
        ...
        status_t status = fCurrent->GetNextMatching(fVolume, fIterator, a_dirent, size);
		if (status < B_OK) {
			delete fIterator;
			fIterator = NULL;
			fCurrent = NULL;
...
void 
Query::SetLiveMode(port_id port, int32_t token)
{
	fPort = port;
	fToken = token;
...
void 
Query::LiveUpdate(Inode *inode, const char *attribute, int32_t type, const uint8_t *oldKey,
