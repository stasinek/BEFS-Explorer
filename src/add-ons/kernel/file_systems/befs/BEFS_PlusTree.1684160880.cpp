...
class NodeChecker {
	public:
		NodeChecker(const bplustree_node *node, int32_t nodeSize, const char *text)
			:
			fNode(node),
			fSize(nodeSize),
			fText(text)
...
// Since BEFS supports block sizes of 1024 bytes or greater, and the node size
...
CachedNode::SetTo(int64_t offset, bool check)
{
	if (fTree == NULL || fTree->fStream == NULL) {
	...
	if (InternalSetTo(NULL, offset) != NULL && check) {
		...
		if (!fTree->fHeader->CheckNode(fNode)) {
			FATAL(("invalid node (%p) read from offset %I64d, inode at %I64d\n",
...
//---------------------------------------------------------------------------
bplustree_node *
CachedNode::SetToWritable(Transaction &transaction, int64_t offset, bool check)
{
	if (fTree == NULL || fTree->fStream == NULL) {
		REPORT_ERROR(B_BAD_VALUE);
		return NULL;
...
CachedNode::SetToWritableHeader(Transaction &transaction)
{
	...
	InternalSetTo(&transaction, 0);
	return (bplustree_header *)fNode;
}
//---------------------------------------------------------------------------
bplustree_node *
CachedNode::InternalSetTo(Transaction *transaction, int64_t offset)
{
	fNode = NULL;
	fOffset = offset;
	...
	if (offset < fTree->fStream->Size()
		...
		int32_t blockOffset = (offset - fileOffset) / volume->BlockSize();
		fBlockNumber = volume->ToBlock(run) + blockOffset;
		uint8_t *block;

		if (transaction != NULL) {
			block = (uint8_t *)block_cache_get_writable(volume->BlockCache(),
				fBlockNumber, transaction->ID());
			fWritable = true;
		} else {
			block = (uint8_t *)block_cache_get(volume->BlockCache(), fBlockNumber);
			fWritable = false;
		}
...
//---------------------------------------------------------------------------
status_t
CachedNode::Free(Transaction &transaction, int64_t offset)
{
    ...
    int64_t lastOffset = header->MaximumSize() - fTree->fNodeSize;
	if (offset == lastOffset) {
		status_t status = fTree->fStream->SetFileSize(transaction, lastOffset);
		if (status < B_OK)
			return status;

		header->maximum_size = HOST_ENDIAN_TO_BEFS_INT64(lastOffset);
		return B_OK;
	}

	// add the node to the free nodes list
	fNode->left_link = header->free_node_pointer;
    fNode->overflow_link = HOST_ENDIAN_TO_BEFS_INT64((int64_t)BPLUSTREE_FREE);

	header->free_node_pointer = HOST_ENDIAN_TO_BEFS_INT64(offset);
	return B_OK;
}
//---------------------------------------------------------------------------
status_t
    ...
    int64_t *_offset)
{
    ...
    int64_t offset = header->MaximumSize();
	header->maximum_size = HOST_ENDIAN_TO_BEFS_INT64(header->MaximumSize()
		+ fTree->fNodeSize);
...
//---------------------------------------------------------------------------
//	#pragma mark -
BPlusTree::BPlusTree(Transaction &transaction, Inode *stream, int32_t nodeSize)
	:
	fStream(NULL),
	fHeader(NULL),
	fCachedHeader(this)
...
status_t
BPlusTree::SetTo(Transaction &transaction, Inode *stream, int32_t nodeSize)
{
	...
	// initialize b+tree header
 	header->magic = HOST_ENDIAN_TO_BEFS_INT32(BPLUSTREE_MAGIC);
 	header->node_size = HOST_ENDIAN_TO_BEFS_INT32(fNodeSize);
 	header->max_number_of_levels = HOST_ENDIAN_TO_BEFS_INT32(1);
 	header->data_type = HOST_ENDIAN_TO_BEFS_INT32(ModeToKeyType(stream->Mode()));
 	header->root_node_pointer = HOST_ENDIAN_TO_BEFS_INT64(nodeSize);
    header->free_node_pointer = HOST_ENDIAN_TO_BEFS_INT64((int64_t)BPLUSTREE_NULL);
 	header->maximum_size = HOST_ENDIAN_TO_BEFS_INT64(nodeSize * 2);
...
BPlusTree::SetTo(Inode *stream)
{
	...
	if (fHeader->MaximumSize() != stream->Size()) {
		FATAL(("B+tree header size %I64d doesn't fit file size %I64d!\n",
			fHeader->MaximumSize(), stream->Size()));
		// we can't change the header since we don't have a transaction
		//fHeader->maximum_size = HOST_ENDIAN_TO_BEFS_INT64(stream->Size());
	}
...
//#endif
	...
	{
		uint32_t toMode[] = {S_STR_INDEX, S_INT_INDEX, S_UINT_INDEX,
			S_LONG_LONG_INDEX, S_ULONG_LONG_INDEX, S_FLOAT_INDEX,
			S_DOUBLE_INDEX};
		uint32_t mode = stream->Mode() & (S_STR_INDEX | S_INT_INDEX
		...
		// in the original BEFS code - we will honour it nevertheless
		fAllowDuplicates = ((stream->Mode() & S_INDEX_DIR) == S_INDEX_DIR
			&& stream->BlockRun() != stream->Parent())
			|| (stream->Mode() & S_ALLOW_DUPS) != 0;
	}
...
//	#pragma mark -
void
BPlusTree::_UpdateIterators(int64_t offset, int64_t nextOffset, uint16_t keyIndex,
	uint16_t splitAt, int8_t change)
{
...
//---------------------------------------------------------------------------
status_t
BPlusTree::_FindKey(const bplustree_node *node, const uint8_t *key,
    uint16_t keyLength, uint16_t *_index, int64_t *_next)
{
#ifdef DEBUG
	...
	for (int16_t first = 0, last = node->NumKeys() - 1; first <= last;) {
		uint16_t i = (first + last) >> 1;

		uint16_t searchLength;
		uint8_t *searchKey = node->KeyAt(i, &searchLength);
        if (searchKey + searchLength + sizeof(int64_t) + sizeof(uint16_t)
				> (uint8_t *)node + fNodeSize
			|| searchLength > BPLUSTREE_MAX_KEY_LENGTH) {
			fStream->GetVolume()->Panic();
			RETURN_ERROR(B_BAD_DATA);
		}

		int32_t cmp = _CompareKeys(key, keyLength, searchKey, searchLength);
		if (cmp < 0) {
			last = i - 1;
			saveIndex = i;
		} else if (cmp > 0) {
			...
			if (_index)
				*_index = i;
			if (_next)
				*_next = BEFS_ENDIAN_TO_HOST_INT64(values[i]);
			return B_OK;
		}
	}
	...
	if (_next) {
		if (saveIndex == node->NumKeys())
			...
			*_next = BEFS_ENDIAN_TO_HOST_INT64(values[saveIndex]);
	}
	return B_ENTRY_NOT_FOUND;
}
//---------------------------------------------------------------------------
...
*/
status_t
BPlusTree::_SeekDown(Stack<node_and_key> &stack, const uint8_t *key,
	uint16_t keyLength)
{
	...
	FATAL(("BPlusTree::SeekDown() could not open node %I64d\n",
...
BPlusTree::_FindFreeDuplicateFragment(Transaction &transaction,
    ...
    int64_t *_offset, bplustree_node **_fragment, uint32_t *_index)
{
    int64_t *values = node->Values();
	for (int32_t i = 0; i < node->NumKeys(); i++) {
        int64_t value = BEFS_ENDIAN_TO_HOST_INT64(values[i]);
		...
		if (fragment == NULL) {
			FATAL(("Could not get duplicate fragment at %I64d\n", value));
			continue;
		}

		// see if there is some space left for us
		uint32_t num = bplustree_node::MaxFragments(fNodeSize);
...
BPlusTree::_InsertDuplicate(Transaction &transaction, CachedNode &cached,
    const bplustree_node *node, uint16_t index, int64_t value)
{
	CachedNode cachedDuplicate(this);
    int64_t *values = node->Values();
    int64_t oldValue = BEFS_ENDIAN_TO_HOST_INT64(values[index]);
	status_t status;
    int64_t offset;
	...
	if (bplustree_node::IsDuplicate(oldValue)) {
		...
		if (bplustree_node::LinkType(oldValue) == BPLUSTREE_DUPLICATE_FRAGMENT) {
			...
			if (array->count > NUM_FRAGMENT_VALUES
				|| array->count < 1) {
				FATAL(("insertDuplicate: Invalid array[%d] size in fragment %I64d == %I64d!\n",
			...
			if (array->count < NUM_FRAGMENT_VALUES) {
				...
				if (duplicate->FragmentsUsed(fNodeSize) < 2) {
					...
					memmove(duplicate->DuplicateArray(), array,
                        (NUM_FRAGMENT_VALUES + 1) * sizeof(int64_t));
					duplicate->left_link = duplicate->right_link
                        = HOST_ENDIAN_TO_BEFS_INT64((int64_t)BPLUSTREE_NULL);
					...
					newDuplicate->overflow_link = HOST_ENDIAN_TO_BEFS_INT64(
						array->count);
					memcpy(&newDuplicate->all_key_count, &array->values[0],
                        array->count * sizeof(int64_t));
                    memset(array, 0, (NUM_FRAGMENT_VALUES + 1) * sizeof(int64_t));
				...
				values[index] = HOST_ENDIAN_TO_BEFS_INT64(bplustree_node::MakeLink(
		...
		do {
			...
			if (array->count > NUM_DUPLICATE_VALUES || array->count < 0) {
				FATAL(("removeDuplicate: Invalid array size in duplicate %I64d == %I64d!\n",
		...
		if (array->count < NUM_DUPLICATE_VALUES) {
			...
			writableDuplicate->right_link = HOST_ENDIAN_TO_BEFS_INT64(offset);
			newDuplicate->left_link = HOST_ENDIAN_TO_BEFS_INT64(duplicateOffset);
...
//---------------------------------------------------------------------------
void
BPlusTree::_InsertKey(bplustree_node *node, uint16_t index, uint8_t *key,
    uint16_t keyLength, int64_t value)
{
    ...
    int64_t *values = node->Values();
	uint16_t *keyLengths = node->KeyLengths();
	uint8_t *keys = node->Keys();

	node->all_key_count = HOST_ENDIAN_TO_BEFS_INT16(node->NumKeys() + 1);
	node->all_key_length = HOST_ENDIAN_TO_BEFS_INT16(node->AllKeyLength()
		+ keyLength);

    int64_t *newValues = node->Values();
	uint16_t *newKeyLengths = node->KeyLengths();

	// move values and copy new value into them
	memmove(newValues + index + 1, values + index,
        sizeof(int64_t) * (node->NumKeys() - 1 - index));
    memmove(newValues, values, sizeof(int64_t) * index);

	newValues[index] = HOST_ENDIAN_TO_BEFS_INT64(value);

	// move and update key length index
	for (uint16_t i = node->NumKeys(); i-- > index + 1;) {
		newKeyLengths[i] = HOST_ENDIAN_TO_BEFS_INT16(
			BEFS_ENDIAN_TO_HOST_INT16(keyLengths[i - 1]) + keyLength);
	}
	memmove(newKeyLengths, keyLengths, sizeof(uint16_t) * index);

	int32_t keyStart;
	newKeyLengths[index] = HOST_ENDIAN_TO_BEFS_INT16(keyLength
		+ (keyStart = index > 0
			? BEFS_ENDIAN_TO_HOST_INT16(newKeyLengths[index - 1]) : 0));

	// move keys and copy new key into them
	uint16_t length = BEFS_ENDIAN_TO_HOST_INT16(newKeyLengths[index]);
	int32_t size = node->AllKeyLength() - length;
	if (size > 0)
...
*/
status_t
BPlusTree::_SplitNode(bplustree_node *node, int64_t nodeOffset,
    bplustree_node *other, int64_t otherOffset, uint16_t *_keyIndex, uint8_t *key,
    uint16_t *_keyLength, int64_t *_value)
{
	if (*_keyIndex > node->NumKeys() + 1)
		return B_BAD_VALUE;

	uint16_t *inKeyLengths = node->KeyLengths();
    int64_t *inKeyValues = node->Values();
	uint8_t *inKeys = node->Keys();
	uint8_t *outKeys = other->Keys();
	int32_t keyIndex = *_keyIndex;	// can become less than zero!
	...
	int32_t bytes = 0, bytesBefore = 0, bytesAfter = 0;
	...
	for (in = out = 0; in < node->NumKeys() + 1;) {
		if (!bytes) {
			bytesBefore = in > 0
				? BEFS_ENDIAN_TO_HOST_INT16(inKeyLengths[in - 1]) : 0;
		}
		...
		if (in == keyIndex && !bytes) {
			...
			if (keyIndex < out) {
				bytesAfter = BEFS_ENDIAN_TO_HOST_INT16(inKeyLengths[in])
					- bytesBefore;
			}
		...
		if (round_up(sizeof(bplustree_node) + bytesBefore + bytesAfter + bytes)
                + out * (sizeof(uint16_t) + sizeof(int64_t)) >= size) {
	...
	if (keyIndex >= out && in > 0)
		bytesBefore = BEFS_ENDIAN_TO_HOST_INT16(inKeyLengths[in - 1]);

	if (bytesBefore < 0 || bytesAfter < 0)
		return B_BAD_DATA;

	other->left_link = node->left_link;
	other->right_link = HOST_ENDIAN_TO_BEFS_INT64(nodeOffset);
	other->all_key_length = HOST_ENDIAN_TO_BEFS_INT16(bytes + bytesBefore
		+ bytesAfter);
	other->all_key_count = HOST_ENDIAN_TO_BEFS_INT16(out);

	uint16_t *outKeyLengths = other->KeyLengths();
    int64_t *outKeyValues = other->Values();
	int32_t keys = out > keyIndex ? keyIndex : out;

	if (bytesBefore) {
		// copy the keys
		memcpy(outKeys, inKeys, bytesBefore);
		memcpy(outKeyLengths, inKeyLengths, keys * sizeof(uint16_t));
        memcpy(outKeyValues, inKeyValues, keys * sizeof(int64_t));
	}
	if (bytes) {
		// copy the newly inserted key
		memcpy(outKeys + bytesBefore, key, bytes);
		outKeyLengths[keyIndex] = HOST_ENDIAN_TO_BEFS_INT16(bytes + bytesBefore);
		outKeyValues[keyIndex] = HOST_ENDIAN_TO_BEFS_INT64(*_value);
		...
		if (bytesAfter) {
			...
			for (int32_t i = 0;i < keys;i++) {
				outKeyLengths[keyIndex + i + 1] = HOST_ENDIAN_TO_BEFS_INT16(
					BEFS_ENDIAN_TO_HOST_INT16(inKeyLengths[keyIndex + i]) + bytes);
			}
	...
	int32_t total = bytesBefore + bytesAfter;
	...
	if (node->OverflowLink() != BPLUSTREE_NULL) {
		if (in == keyIndex) {
			...
			other->overflow_link = HOST_ENDIAN_TO_BEFS_INT64(*_value);
			keyIndex--;
		} else {
			// If a key is dropped (is not the new key), we have to copy
			// it, because it would be lost if not.
			uint8_t *droppedKey = node->KeyAt(in, &newLength);
            if (droppedKey + newLength + sizeof(int64_t) + sizeof(uint16_t)
					> (uint8_t *)node + fNodeSize
			...
			total = BEFS_ENDIAN_TO_HOST_INT16(inKeyLengths[in++]);
		}
	}
	...
	while (in < node->NumKeys() + 1) {
		if (in == keyIndex && !bytes) {
			...
			if (in < node->NumKeys()) {
				inKeyLengths[in] = HOST_ENDIAN_TO_BEFS_INT16(
					BEFS_ENDIAN_TO_HOST_INT16(inKeyLengths[in]) - total);

				if (bytes) {
					inKeyLengths[in] = HOST_ENDIAN_TO_BEFS_INT16(
						BEFS_ENDIAN_TO_HOST_INT16(inKeyLengths[in]) + bytes);
	...
	node->left_link = HOST_ENDIAN_TO_BEFS_INT64(otherOffset);
		// right link, and overflow link can stay the same
	node->all_key_length = HOST_ENDIAN_TO_BEFS_INT16(bytes + bytesBefore
		+ bytesAfter);
	node->all_key_count = HOST_ENDIAN_TO_BEFS_INT16(out - 1);
	...
	if (bytesBefore)
		memmove(outKeyLengths, inKeyLengths + skip, keys * sizeof(uint16_t));
	in = out - keyIndex - 1;
	if (bytesAfter) {
	...
	if (bytesBefore)
        memmove(outKeyValues, inKeyValues + skip, keys * sizeof(int64_t));
	if (bytesAfter) {
...
*/
status_t
BPlusTree::Insert(Transaction &transaction, const uint8_t *key, uint16_t keyLength,
    int64_t value)
{
...
#ifdef DEBUG
	if (value < 0)
		panic("tried to insert invalid value %I64d!\n", value);
#endif
	...
	uint8_t keyBuffer[BPLUSTREE_MAX_KEY_LENGTH + 1];
	...
	while (stack.Pop(&nodeAndKey)
		...
		if (int32_t(round_up(sizeof(bplustree_node)
				+ writableNode->AllKeyLength() + keyLength)
				+ (writableNode->NumKeys() + 1) * (sizeof(uint16_t)
                + sizeof(int64_t))) < fNodeSize) {
			...
			// it now
            int64_t newRoot = BPLUSTREE_NULL;
			...
			if ((other = cachedOther.SetToWritable(transaction,
					other->LeftLink())) != NULL) {
				other->right_link = HOST_ENDIAN_TO_BEFS_INT64(otherOffset);
			}
			...
			if (newRoot != BPLUSTREE_NULL) {
				...
				root->overflow_link = HOST_ENDIAN_TO_BEFS_INT64(
					nodeAndKey.nodeOffset);
				...
				header->root_node_pointer = HOST_ENDIAN_TO_BEFS_INT64(newRoot);
				header->max_number_of_levels = HOST_ENDIAN_TO_BEFS_INT32(
...
BPlusTree::_RemoveDuplicate(Transaction &transaction,
	const bplustree_node *node, CachedNode &cached, uint16_t index,
    int64_t value)
{
    int64_t *values = node->Values();
    int64_t oldValue = BEFS_ENDIAN_TO_HOST_INT64(values[index]);
	...
	CachedNode cachedDuplicate(this);
    int64_t duplicateOffset = bplustree_node::FragmentOffset(oldValue);
	bplustree_node *duplicate = cachedDuplicate.SetToWritable(transaction,
	...
	if (bplustree_node::LinkType(oldValue) == BPLUSTREE_DUPLICATE_FRAGMENT) {
		...
		if (array->count > NUM_FRAGMENT_VALUES
			|| array->count < 1) {
			FATAL(("removeDuplicate: Invalid array[%d] size in fragment %I64d == %I64d!\n",
				(int)bplustree_node::FragmentIndex(oldValue), duplicateOffset, array->count));
			return B_BAD_DATA;
		}
		if (!array->Remove(value)) {
			FATAL(("Oh no, value %I64d not found in fragments of node %I64d...\n",
	...
	if (duplicate->LeftLink() != BPLUSTREE_NULL) {
		FATAL(("invalid duplicate node: first left link points to %I64d!\n",
			duplicate->LeftLink()));
		return B_BAD_DATA;
	}
	...
	while (true) {
        int64_t left = duplicate->LeftLink();
        int64_t right = duplicate->RightLink();
		...
		if ((isLast && array->count == 1) || array->count == 0) {
			...
			if (left != BPLUSTREE_NULL
				...
				duplicate->right_link = HOST_ENDIAN_TO_BEFS_INT64(right);
			...
			if (right != BPLUSTREE_NULL
				...
				duplicate->left_link = HOST_ENDIAN_TO_BEFS_INT64(left);
			...
			if (_FindFreeDuplicateFragment(transaction, node, cachedOther,
                ...
                memcpy(target, array, (NUM_FRAGMENT_VALUES + 1) * sizeof(int64_t));

				cachedDuplicate.Free(transaction, duplicateOffset);
				duplicateOffset = offset;
			} else {
				// convert node
                memmove(duplicate, array, (NUM_FRAGMENT_VALUES + 1) * sizeof(int64_t));
                memset((int64_t *)duplicate + NUM_FRAGMENT_VALUES + 1, 0,
                    fNodeSize - (NUM_FRAGMENT_VALUES + 1) * sizeof(int64_t));
			}
...
*/
void
BPlusTree::_RemoveKey(bplustree_node *node, uint16_t index)
{
	// should never happen, but who knows?
	...
	uint16_t length;
	uint8_t *key = node->KeyAt(index, &length);
    if (key + length + sizeof(int64_t) + sizeof(uint16_t) > (uint8_t *)node + fNodeSize
	...
	node->all_key_count = HOST_ENDIAN_TO_BEFS_INT16(node->NumKeys() - 1);
	node->all_key_length = HOST_ENDIAN_TO_BEFS_INT64(node->AllKeyLength() - length);
	...
	if (index > 0 && newKeyLengths != keyLengths)
		memmove(newKeyLengths, keyLengths, index * sizeof(uint16_t));
	for (uint16_t i = index; i < node->NumKeys(); i++) {
		newKeyLengths[i] = HOST_ENDIAN_TO_BEFS_INT16(
			BEFS_ENDIAN_TO_HOST_INT16(keyLengths[i + 1]) - length);
	}

	// move values
	if (index > 0)
        memmove(newValues, values, index * sizeof(int64_t));
	if (node->NumKeys() > index)
        memmove(newValues + index, values + index + 1, (node->NumKeys() - index) * sizeof(int64_t));
}
//---------------------------------------------------------------------------
/*!
...
*/
status_t
BPlusTree::Remove(Transaction &transaction, const uint8_t *key, uint16_t keyLength,
    int64_t value)
{
	if (keyLength < BPLUSTREE_MIN_KEY_LENGTH
		|| keyLength > BPLUSTREE_MAX_KEY_LENGTH)
	...
	while (stack.Pop(&nodeAndKey)
		...
		if (node->IsLeaf()) {
            ...
            int64_t next = node->RightLink() == BPLUSTREE_NULL
			...
			// is this a duplicate entry?
			if (bplustree_node::IsDuplicate(BEFS_ENDIAN_TO_HOST_INT64(
        ...
        if ((nodeAndKey.nodeOffset == fHeader->RootNode()
            ...
            writableNode->overflow_link = HOST_ENDIAN_TO_BEFS_INT64((int64_t)BPLUSTREE_NULL);
...
*/
status_t
BPlusTree::Replace(Transaction &transaction, const uint8_t *key,
    uint16_t keyLength, int64_t value)
{
	if (keyLength < BPLUSTREE_MIN_KEY_LENGTH
		|| keyLength > BPLUSTREE_MAX_KEY_LENGTH
    ...
    int64_t nodeOffset = fHeader->RootNode();
	CachedNode cached(this);
	const bplustree_node *node;
	...
	while ((node = cached.SetTo(nodeOffset)) != NULL) {
		...
		if (node->OverflowLink() == BPLUSTREE_NULL) {
			if (status == B_OK) {
				bplustree_node *writableNode = cached.MakeWritable(transaction);
				if (writableNode != NULL)
					writableNode->Values()[keyIndex] = HOST_ENDIAN_TO_BEFS_INT64(value);
				else
					status = B_IO_ERROR;
			}
...
*/
status_t
BPlusTree::Find(const uint8_t *key, uint16_t keyLength, int64_t *_value)
{
	if (keyLength < BPLUSTREE_MIN_KEY_LENGTH
		|| keyLength > BPLUSTREE_MAX_KEY_LENGTH
		|| key == NULL)
	...
	while ((node = cached.SetTo(nodeOffset)) != NULL) {
		...
		if (node->OverflowLink() == BPLUSTREE_NULL) {
			if (status == B_OK && _value != NULL)
				*_value = BEFS_ENDIAN_TO_HOST_INT64(node->Values()[keyIndex]);

#ifdef DEBUG
			if (levels != (int32_t)fHeader->MaxNumberOfLevels())
	...
	FATAL(("b+tree node at %I64d could not be loaded\n", nodeOffset));
	RETURN_ERROR(B_ERROR);
}
...
TreeIterator::Goto(int8_t to)
{
    ...
    int64_t nodeOffset = fTree->fHeader->RootNode();
	...
	while ((node = cached.SetTo(nodeOffset)) != NULL) {
		...
		if (to == BPLUSTREE_END || node->all_key_count == 0)
			...
			if (node->AllKeyLength() > fTree->fNodeSize
				|| (uint32_t)node->Values() > (uint32_t)node + fTree->fNodeSize
					- 8 * node->NumKeys())
				RETURN_ERROR(B_ERROR);

			nextOffset = BEFS_ENDIAN_TO_HOST_INT64(node->Values()[0]);
		}
...
*/
status_t
TreeIterator::Traverse(int8_t direction, void *key, uint16_t *keyLength,
    uint16_t maxLength, int64_t *value, uint16_t *duplicate)
{
...
//	printf("TreeIterator::Traverse 8\n");
    int64_t savedNodeOffset = fCurrentNodeOffset;
	int32_t savedKey = fCurrentKey;
...
//	printf("TreeIterator::Traverse 11 - fCurrentKey=%i\n", fCurrentKey);
	...
	uint16_t length;
	uint8_t *keyStart = node->KeyAt(fCurrentKey, &length);
    if (keyStart + length + sizeof(int64_t) + sizeof(uint16_t) > (uint8_t *)node + fTree->fNodeSize
		|| length > BPLUSTREE_MAX_KEY_LENGTH) {
...
//	printf("TreeIterator::Traverse 13\n");
    int64_t offset = BEFS_ENDIAN_TO_HOST_INT64(node->Values()[fCurrentKey]);

	// duplicate fragments?
	uint8_t type = bplustree_node::LinkType(offset);
...
*/
status_t 
TreeIterator::Find(const uint8_t *key, uint16_t keyLength)
{
	if (fTree == NULL)
		return B_INTERRUPTED;
...
//---------------------------------------------------------------------------
void
TreeIterator::Update(int64_t offset, int64_t nextOffset, uint16_t keyIndex, uint16_t splitAt, int8_t change)
{
...
TreeIterator::Dump()
{
	...
	__out("\tfTree = %p\n", fTree);
	__out("\tfCurrentNodeOffset = %I64d\n", fCurrentNodeOffset);
	__out("\tfCurrentKey = %I64d\n", fCurrentKey);
    __out("\tfDuplicateNode = %I64d (%I64d, 0x%I64d)\n", bplustree_node::FragmentOffset(fDuplicateNode), fDuplicateNode, fDuplicateNode);
	__out("\tfDuplicate = %u\n", fDuplicate);
...
{
    left_link = right_link = overflow_link = HOST_ENDIAN_TO_BEFS_INT64((int64_t)BPLUSTREE_NULL);
	all_key_count = 0;
	all_key_length = 0;
}
//---------------------------------------------------------------------------
uint8_t *
bplustree_node::KeyAt(int32_t index, uint16_t *keyLength) const
{
	if (index < 0 || index > NumKeys())
		return NULL;

	uint8_t *keyStart = Keys();
	uint16_t *keyLengths = KeyLengths();

	*keyLength = BEFS_ENDIAN_TO_HOST_INT16(keyLengths[index])
		- (index != 0 ? BEFS_ENDIAN_TO_HOST_INT16(keyLengths[index - 1]) : 0);
	if (index > 0)
		keyStart += BEFS_ENDIAN_TO_HOST_INT16(keyLengths[index - 1]);

	return keyStart;
}
//---------------------------------------------------------------------------
uint8_t
bplustree_node::CountDuplicates(int64_t offset, bool isFragment) const
{
	// the duplicate fragment handling is currently hard-coded to a node size
	// of 1024 bytes - with future versions of BEFS, this may be a problem

	if (isFragment) {
        uint32_t fragment = (NUM_FRAGMENT_VALUES + 1) * ((int64_t)offset & 0x3ff);

        return ((int64_t *)this)[fragment];
	}
	return OverflowLink();
}
//---------------------------------------------------------------------------
int64_t
bplustree_node::DuplicateAt(int64_t offset, bool isFragment, int8_t index) const
{
	uint32_t start;
	if (isFragment)
        start = 8 * ((int64_t)offset & 0x3ff);
	else
		start = 2;

    return ((int64_t *)this)[start + 1 + index];
}
//---------------------------------------------------------------------------
...
*/
uint32_t
bplustree_node::FragmentsUsed(uint32_t nodeSize) const
{
	uint32_t used = 0;
	for (uint32_t i = 0; i < MaxFragments(nodeSize); i++) {
...
#ifdef DEBUG
status_t
bplustree_node::CheckIntegrity(uint32_t nodeSize) const
{
	...
	for (int32_t i = 0; i < NumKeys(); i++) {
		uint16_t length;
		uint8_t *key = KeyAt(i, &length);
        if (key + length + sizeof(int64_t) + sizeof(uint16_t) > (uint8_t *)this + nodeSize
			|| length > BPLUSTREE_MAX_KEY_LENGTH) {
			printf("node %p, key %I64d\n", this, i);
			DEBUGGER(("invalid node: keys corrupted"));
			return B_BAD_DATA;
		}
		if (Values()[i] == -1) {
			printf("node %p, value %I64d: %I64d\n", this, i, Values()[i]);
