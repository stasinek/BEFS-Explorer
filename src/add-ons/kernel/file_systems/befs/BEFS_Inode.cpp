...
void fill_stat_buffer(Inode *inode, struct attr_stat &stat)
{
	...
	stat.st_nlink = 1;
//	stat.st_blksize = BEFS_IO_SIZE; // TODO: what whith this?
...
InodeAllocator::~InodeAllocator()
{
	if (fTransaction != NULL) {
		...
		if (fInode != NULL) {
			fInode->Node().flags &= ~HOST_ENDIAN_TO_BEFS_INT32(INODE_IN_USE);
...
status_t InodeAllocator::New(block_run *parentRun, mode_t mode, block_run &run, Inode **_inode)
{
	Volume *volume = fTransaction->GetVolume();
...
status_t InodeAllocator::CreateTree()
{
	...
	if ((fInode->Mode() & S_INDEX_TYPES) == 0)
		fInode->Node().mode |= HOST_ENDIAN_TO_BEFS_INT32(S_STR_INDEX);
...
status_t InodeAllocator::Keep()
{
	...
	if (status < B_OK) {
		FATAL(("writing new inode %I64d failed!\n", fInode->ID()));
		return status;
	}
...
status_t bfs_inode::InitCheck(Volume *volume)
{
	...
	if (Magic1() != INODE_MAGIC1
		...
		|| inode_num.Length() != 1
		// matches inode size?
		|| (uint32_t)InodeSize() != volume->InodeSize()
		// parent resides on disk?
		|| parent.AllocationGroup() > int32_t(volume->AllocationGroups())
		|| parent.AllocationGroup() < 0
		|| parent.Start() > (1L << volume->AllocationGroupShift())
		|| parent.Length() != 1
		// attributes, too?
		|| attributes.AllocationGroup() > int32_t(volume->AllocationGroups())
...
Inode::Inode(Volume *volume, vnode_id id)
	...
	fCache(NULL)
{
	//printf("Inode::Inode(volume = %p, id = %I64d) @ %p\n", volume, id, this);
...
Inode::Inode(Volume *volume, Transaction &transaction, vnode_id id, mode_t mode, block_run &run)
	...
	fCache(NULL)
{
	//PRINT(("Inode::Inode(volume = %p, transaction = %p, id = %I64d) @ %p\n",
	...
	Node().magic1 = HOST_ENDIAN_TO_BEFS_INT32(INODE_MAGIC1);
	Node().inode_num = run;
	Node().mode = HOST_ENDIAN_TO_BEFS_INT32(mode);
	Node().flags = HOST_ENDIAN_TO_BEFS_INT32(INODE_IN_USE);

	Node().create_time = HOST_ENDIAN_TO_BEFS_INT64((bigtime_t)time(NULL) << INODE_TIME_SHIFT);
	Node().last_modified_time = HOST_ENDIAN_TO_BEFS_INT64(Node().create_time
		| (volume->GetUniqueID() & INODE_TIME_MASK));
		// we use Volume::GetUniqueID() to avoid having too many duplicates in the
		// last_modified index

	Node().inode_size = HOST_ENDIAN_TO_BEFS_INT32(volume->InodeSize());
...
status_t Inode::InitCheck(bool checkNode)
{
	...
	if (checkNode) {
		...
		if (status < B_OK) {
			FATAL(("inode at block %I64d corrupt!\n", BlockNumber()));
			RETURN_ERROR(B_BAD_DATA);
		}
	}
	...
	if (IsContainer()) {
		...
		if (status < B_OK) {
			FATAL(("inode tree at block %I64d corrupt!\n", BlockNumber()));
			RETURN_ERROR(B_BAD_DATA);
		}
	}
...
status_t Inode::WriteBack(Transaction &transaction)
{
...
status_t Inode::CheckPermissions(int accessMode) const
{
	uid_t user = geteuid();
	gid_t group = getegid();
...
void Inode::_AddIterator(AttributeIterator *iterator)
{
	// TODO: fix locking
...
void Inode::_RemoveIterator(AttributeIterator *iterator)
{
	if (fSmallDataLock.Lock() < B_OK)
		return;
...
status_t Inode::_MakeSpaceForSmallData(Transaction &transaction, bfs_inode *node,
	const char *name, int32_t bytes)
{
	ASSERT(fSmallDataLock.IsLocked());

	while (bytes > 0) {
		small_data *item = node->SmallDataStart(), *max = NULL;
		int32_t index = 0, maxIndex = 0;
		for (; !item->IsLast(node); item = item->Next(), index++) {
			...
			if (bytes < (int32_t)item->Size())
				break;
		}

		if (item->IsLast(node) || (int32_t)item->Size() < bytes)
			return B_ERROR;
...
status_t Inode::_RemoveSmallData(bfs_inode *node, small_data *item, int32_t index)
{
	ASSERT(fSmallDataLock.IsLocked());
	...
	if (!next->IsLast(node)) {
		...
		int32_t size = (uint8_t *)last - (uint8_t *)next;
		if (size < 0 || size > (uint8_t *)node + fVolume->BlockSize() - (uint8_t *)next)
			return B_BAD_DATA;
		...
		last = (small_data *)((uint8_t *)last - ((uint8_t *)next - (uint8_t *)item));
		memset(last, 0, (uint8_t *)node + fVolume->BlockSize() - (uint8_t *)last);
	} else
...
status_t Inode::_RemoveSmallData(Transaction &transaction, NodeGetter &nodeGetter,
	const char *name)
{
	if (name == NULL)
		return B_BAD_VALUE;
...
status_t Inode::_AddSmallData(Transaction &transaction, NodeGetter &nodeGetter,
	const char *name, uint32_t type, const uint8_t *data, size_t length, bool force)
{
	bfs_inode *node = nodeGetter.WritableNode();
	...
	uint32_t spaceNeeded = sizeof(small_data) + nameLength + 3 + length + 1;
	...
	if (!item->IsLast(node)) {
		...
		if (item->data_size > length
			|| force
			|| ((uint8_t *)last + length - item->DataSize()) <= ((uint8_t *)node + fVolume->InodeSize())) {
			...
			if (force
				...
				uint32_t needed = length - item->DataSize() -
						(uint32_t)((uint8_t *)node + fVolume->InodeSize() - (uint8_t *)last);
			...
			if (length != item->DataSize()) {
				...
				small_data *next = item->Next();
				if (!next->IsLast(node))
					memmove((uint8_t *)item + spaceNeeded, next, (uint8_t *)last - (uint8_t *)next);

				// Move the "last" one to its new location and
				// correctly terminate the small_data section
				last = (small_data *)((uint8_t *)last - ((uint8_t *)next - ((uint8_t *)item + spaceNeeded)));
				if ((uint8_t *)last < (uint8_t *)node + fVolume->BlockSize())
					memset(last, 0, (uint8_t *)node + fVolume->BlockSize() - (uint8_t *)last);

				item->data_size = HOST_ENDIAN_TO_BEFS_INT16(length);
			}

			item->type = HOST_ENDIAN_TO_BEFS_INT32(type);
	...
	if ((uint8_t *)item + spaceNeeded > (uint8_t *)node + fVolume->InodeSize()) {
	...
	item->type = HOST_ENDIAN_TO_BEFS_INT32(type);
	item->name_size = HOST_ENDIAN_TO_BEFS_INT16(nameLength);
	item->data_size = HOST_ENDIAN_TO_BEFS_INT16(length);
	...
	item = item->Next();
	if (!item->IsLast(node))
		memset(item, 0, (uint8_t *)node + fVolume->InodeSize() - (uint8_t *)item);
...
status_t Inode::_GetNextSmallData(bfs_inode *node, small_data **_smallData) const
{
	if (node == NULL)
...
small_data *Inode::FindSmallData(const bfs_inode *node, const char *name) const
{
	//todo: fix locking
	//ASSERT(fSmallDataLock.IsLocked());
...
const char *Inode::Name(const bfs_inode *node) const
{
	ASSERT(fSmallDataLock.IsLocked());
...
status_t Inode::GetName(char *buffer, size_t size) const
{
...
status_t Inode::SetName(Transaction &transaction, const char *name)
{
	if (name == NULL || *name == '\0')
		return B_BAD_VALUE;
	...
	return _AddSmallData(transaction, node, nameTag, FILE_NAME_TYPE,(uint8_t *)name, strlen(name), true);
}
//-----------------------------------------------------------------------------

status_t Inode::_RemoveAttribute(Transaction &transaction, const char *name,
	bool hasIndex, Index *index)
{
	...
	if (index != NULL) {
		...
		if ((hasIndex || fVolume->CheckForLiveQuery(name))
			...
			uint8_t data[BPLUSTREE_MAX_KEY_LENGTH];
            int64_t length = BPLUSTREE_MAX_KEY_LENGTH;
	...
	if (attributes->IsEmpty()) {
		...
		if (remove_vnode(fVolume->ID(), attributes->ID()) == B_OK) {
			...
			attributes->Node().flags |= HOST_ENDIAN_TO_BEFS_INT32(INODE_DELETED);
...
status_t Inode::ReadAttribute(const char *name, int32_t type, int64_t pos, uint8_t *buffer,
    int64_t *_length)
{
	if (pos < 0)
		pos = 0;
	...
	if (status == B_OK) {
		...
		//if (attribute->Lock().Lock() == B_OK) {
			status = attribute->ReadAt(pos, (uint8_t *)buffer, _length);
...
status_t Inode::WriteAttribute(Transaction &transaction, const char *name, int32_t type,
    int64_t pos, const uint8_t *buffer, size_t *_length)
{
	// needed to maintain the index
	uint8_t oldBuffer[BPLUSTREE_MAX_KEY_LENGTH], *oldData = NULL;
    int64_t oldLength = 0;
		...
		if (attribute->Lock().LockWrite() == B_OK) {
			...
			if (status == B_OK) {
				...
				if (status == B_OK) {
					...
					attribute->Node().type = HOST_ENDIAN_TO_BEFS_INT32(type);
				}
...
status_t Inode::RemoveAttribute(Transaction &transaction, const char *name)
{
	Index index(fVolume);
	...
	// update index for attributes in the small_data section
	{
		...
		small_data *smallData = FindSmallData(node.Node(), name);
		if (smallData != NULL) {
			uint32_t length = smallData->DataSize();
			if (length > BPLUSTREE_MAX_KEY_LENGTH)
				length = BPLUSTREE_MAX_KEY_LENGTH;
...
status_t Inode::GetAttribute(const char *name, Inode **_attribute)
{
	...
	if (status == B_OK) {
		//vnode_id id;
        int64_t fid;//=id;
		status = tree->Find((uint8_t *)name, (uint16_t)strlen(name), &fid);
		if (status == B_OK) {
...
void Inode::ReleaseAttribute(Inode *attribute)
{
	if (attribute == NULL)
		return;
...
status_t Inode::CreateAttribute(Transaction &transaction, const char *name, uint32_t type,
	Inode **attribute)
{
...
status_t Inode::GetTree(BPlusTree **tree)
{
	if (fTree) {
		*tree = fTree;
		return B_OK;
...
status_t Inode::FindBlockRun(int64_t pos, block_run &run, int64_t &offset)
{
	...
	if (data->MaxDirectRange() > 0 && pos >= data->MaxDirectRange()) {
		if (data->MaxDoubleIndirectRange() > 0 && pos >= data->MaxIndirectRange()) {
            ...
            int64_t start = pos - data->MaxIndirectRange();
			int32_t indirectSize = (1L << (INDIRECT_BLOCKS_SHIFT
					+ cached.BlockShift()))
				* (fVolume->BlockSize() / sizeof(block_run));
			int32_t directSize = NUM_ARRAY_BLOCKS << cached.BlockShift();
			int32_t index = start / indirectSize;
			int32_t runsPerBlock = cached.BlockSize() / sizeof(block_run);
			...
			//printf("\tstart = %I64d, indirectSize = %I64d, directSize = %I64d, index = %I64d\n",start,indirectSize,directSize,index);
			//printf("\tlook for indirect block at %I64d,%d\n",indirect[index].allocation_group,indirect[index].start);

			int32_t current = (start % indirectSize) / directSize;
			...
			offset = data->MaxIndirectRange() + (index * indirectSize)
				+ (current * directSize);
			//printf("\tfCurrent = %I64d, fRunFileOffset = %I64d, fRunBlockEnd = %I64d, fRun = %I64d,%d\n",fCurrent,fRunFileOffset,fRunBlockEnd,fRun.allocation_group,fRun.start);
		} else {
			// access to indirect blocks

			int32_t runsPerBlock = fVolume->BlockSize() / sizeof(block_run);
            int64_t runBlockEnd = data->MaxDirectRange();

			CachedBlock cached(fVolume);
            int64_t block = fVolume->ToBlock(data->indirect);

			for (int32_t i = 0; i < data->indirect.Length(); i++) {
				...
				while (++current < runsPerBlock) {
					...
					if (runBlockEnd > pos) {
						...
						offset = runBlockEnd - (run.Length()
							<< cached.BlockShift());
						//printf("reading from indirect block: %I64d,%d\n",fRun.allocation_group,fRun.start);
						//printf("### indirect-run[%I64d] = (%I64d,%d,%d), offset = %I64d\n",fCurrent,fRun.allocation_group,fRun.start,fRun.Length(),fRunFileOffset);
		...
		while (++current < NUM_DIRECT_BLOCKS) {
			...
			if (runBlockEnd > pos) {
				...
				//printf("### run[%I64d] = (%I64d,%d,%d), offset = %I64d\n",fCurrent,fRun.allocation_group,fRun.start,fRun.Length(),fRunFileOffset);
				return fVolume->ValidateBlockRun(run);
			}
		}

		//PRINT(("FindBlockRun() failed in direct range: size = %I64d, pos = %I64d\n",data->size,pos));
		return B_ENTRY_NOT_FOUND;
	}
	return fVolume->ValidateBlockRun(run);
}
...
status_t Inode::ReadAt(int64_t pos, uint8_t *buffer, int64_t *_length)
{
#if 0   //disable code
...
status_t Inode::WriteAt(Transaction &transaction, int64_t pos, const uint8_t *buffer,size_t *_length)
{
	...
	Node().last_modified_time = HOST_ENDIAN_TO_BEFS_INT64((bigtime_t)time(NULL)
		<< INODE_TIME_SHIFT);
...
status_t Inode::FillGapWithZeros(int64_t pos, int64_t newSize)
{
	// ToDo: we currently do anything here, same as original BEFS!
	//if (pos >= newSize)
	return B_OK;
#if 0
	block_run run;
    ...
    int64_t length = newSize - pos;
	uint32_t bytesWritten = 0;
	uint32_t blockSize = fVolume->BlockSize();
	...
	while (length > 0) {
		...
		run.start = HOST_ENDIAN_TO_BEFS_INT16(run.Start() + ((pos - offset) >> blockShift));
		run.length = HOST_ENDIAN_TO_BEFS_INT16(run.Length() - ((pos - offset) >> blockShift));

		CachedBlock cached(fVolume);
        int64_t blockNumber = fVolume->ToBlock(run);
		for (int32_t i = 0; i < run.Length(); i++) {
		...
		int32_t bytes = run.Length() << blockShift;
		length -= bytes;
		bytesWritten += bytes;
...
status_t Inode::_AllocateBlockArray(Transaction &transaction, block_run &run)
{
	if (!run.IsZero())
		return B_BAD_VALUE;
	...
	CachedBlock cached(fVolume);
    int64_t block = fVolume->ToBlock(run);
...
status_t Inode::_GrowStream(Transaction &transaction, int64_t size)
{
	data_stream *data = &Node().data;
	...
	if (size < data->MaxDirectRange()
		...
		data->size = HOST_ENDIAN_TO_BEFS_INT64(size);
		return B_OK;
	}
    ...
    int64_t blocksRequested = (bytes + fVolume->BlockSize() - 1)
		>> fVolume->BlockShift();
	if (blocksRequested > fVolume->FreeBlocks())
		return B_DEVICE_FULL;

    int64_t blocksNeeded = blocksRequested;
	...
	while (blocksNeeded > 0) {
		...
		if (data->Size() <= data->MaxDirectRange()) {
			...
			if (free < NUM_DIRECT_BLOCKS) {
				...
				if (free > 0 && data->direct[last].MergeableWith(run)) {
					data->direct[last].length = HOST_ENDIAN_TO_BEFS_INT16(
						data->direct[last].Length() + run.Length());
				} else
					data->direct[free] = run;

				data->max_direct_range = HOST_ENDIAN_TO_BEFS_INT64(
					data->MaxDirectRange() + run.Length() * fVolume->BlockSize());
				data->size = HOST_ENDIAN_TO_BEFS_INT64(blocksNeeded > 0
		...
		if (data->Size() <= data->MaxIndirectRange()
			...
			if (data->indirect.IsZero()) {
				...
				data->max_indirect_range = HOST_ENDIAN_TO_BEFS_INT64(
					data->MaxDirectRange());
				// insert the block_run in the first block
				runs = (block_run *)cached.SetTo(data->indirect);
			} else {
				uint32_t numberOfRuns = fVolume->BlockSize() / sizeof(block_run);
			...
			if (runs != NULL) {
				...
				if (free > 0 && runs[last].MergeableWith(run)) {
					runs[last].length = HOST_ENDIAN_TO_BEFS_INT16(
		...
		if (data->Size() <= data->MaxDoubleIndirectRange()
			...
			while ((run.Length() % NUM_ARRAY_BLOCKS) != 0) {
				...
				// of NUM_ARRAY_BLOCKS
				int32_t rest = run.Length() % NUM_ARRAY_BLOCKS;
				run.length = HOST_ENDIAN_TO_BEFS_INT16(run.Length() - rest);
			...
			int32_t runsPerBlock = fVolume->BlockSize() / sizeof(block_run);
			int32_t indirectSize = ((1L << INDIRECT_BLOCKS_SHIFT) << fVolume->BlockShift())
				* runsPerBlock;
			int32_t directSize = NUM_ARRAY_BLOCKS << fVolume->BlockShift();
			int32_t runsPerArray = runsPerBlock << ARRAY_BLOCKS_SHIFT;

            int64_t start = data->MaxDoubleIndirectRange() - data->MaxIndirectRange();
			int32_t indirectIndex = start / indirectSize;
			int32_t index = start / directSize;
...
*/
				...
				do {
					...
					do {
						...
						runs[index % runsPerBlock].length
							= HOST_ENDIAN_TO_BEFS_INT16(NUM_ARRAY_BLOCKS);

						// alter the remaining block_run
						run.start = HOST_ENDIAN_TO_BEFS_INT16(run.Start()
							+ NUM_ARRAY_BLOCKS);
						run.length = HOST_ENDIAN_TO_BEFS_INT16(run.Length()
							- NUM_ARRAY_BLOCKS);
			...
			data->max_double_indirect_range = HOST_ENDIAN_TO_BEFS_INT64(
				data->MaxDoubleIndirectRange()
				+ (runLength << fVolume->BlockShift()));
			data->size = blocksNeeded > 0 ? HOST_ENDIAN_TO_BEFS_INT64(
...
status_t Inode::_FreeStaticStreamArray(Transaction &transaction, int32_t level,
    block_run run, int64_t size, int64_t offset, int64_t &max)
{
	int32_t indirectSize = 0;
	if (level == 0)
    ...
    offset += (int64_t)index * indirectSize;

	for (int32_t i = index / runsPerBlock; i < run.Length(); i++) {
		...
		for (index = index % runsPerBlock; index < runsPerBlock; index++) {
			...
			if (level == 0) {
				...
				max = HOST_ENDIAN_TO_BEFS_INT64(offset + indirectSize);
...
status_t Inode::_FreeStreamArray(Transaction &transaction, block_run *array,
    uint32_t arrayLength, int64_t size, int64_t &offset, int64_t &max)
{
	PRINT(("FreeStreamArray: arrayLength %lu, size %I64d, offset %I64d, max %I64d\n",
	...
	for (; i < arrayLength; i++, offset = newOffset) {
        ...
        newOffset += (int64_t)array[i].Length() << fVolume->BlockShift();
		if (newOffset <= size)
			continue;
		...
		if (newOffset > size && offset < size) {
			...
			array[i].length = HOST_ENDIAN_TO_BEFS_INT16(run.Start()
				- array[i].Start());
			run.length = HOST_ENDIAN_TO_BEFS_INT16(run.Length()
				- array[i].Length());
            ...
            max = HOST_ENDIAN_TO_BEFS_INT64(offset + ((int64_t)array[i].Length()
				<< fVolume->BlockShift()));
		} else {
			// free the whole block_run
			array[i].SetTo(0, 0, 0);

            if ((int64_t)BEFS_ENDIAN_TO_HOST_INT64(max) > offset)
				max = HOST_ENDIAN_TO_BEFS_INT64(offset);
		}
...
status_t Inode::_ShrinkStream(Transaction &transaction, int64_t size)
{
	data_stream *data = &Node().data;
	status_t status;

	if (data->MaxDoubleIndirectRange() > size) {
        int64_t *maxDoubleIndirect = &data->max_double_indirect_range;
			// gcc 4 work-around: "error: cannot bind packed field
            // 'data->data_stream::max_double_indirect_range' to 'int64_t&'"
	...
	if (data->MaxIndirectRange() > size) {
		CachedBlock cached(fVolume);
        int64_t block = fVolume->ToBlock(data->indirect);
        int64_t offset = data->MaxDirectRange();
		...
		for (int32_t i = 0; i < data->indirect.Length(); i++) {
            ...
            int64_t *maxIndirect = &data->max_indirect_range;
				// gcc 4 work-around: "error: cannot bind packed field
                // 'data->data_stream::max_indirect_range' to 'int64_t&'"
	...
	if (data->MaxDirectRange() > size) {
        int64_t offset = 0;
        int64_t *maxDirect = &data->max_direct_range;
			// gcc 4 work-around: "error: cannot bind packed field
            // 'data->data_stream::max_direct_range' to 'int64_t&'"
...
status_t Inode::SetFileSize(Transaction &transaction, int64_t size)
{
	if (size < 0)
		return B_BAD_VALUE;
...
status_t Inode::Append(Transaction &transaction, int64_t bytes)
{
...
bool Inode::NeedsTrimming()
{
    ...
    int64_t roundedSize = (Size() + fVolume->BlockSize() - 1)
		& ~(fVolume->BlockSize() - 1);
...
status_t Inode::TrimPreallocation(Transaction &transaction)
{
...
status_t Inode::Free(Transaction &transaction)
{
	FUNCTION();
...
status_t Inode::Sync()
{
	...
	for (int32_t i = 0; i < NUM_DIRECT_BLOCKS; i++) {
	...
	CachedBlock cached(fVolume);
    int64_t block = fVolume->ToBlock(data->indirect);
	int32_t count = fVolume->BlockSize() / sizeof(block_run);

	for (int32_t j = 0; j < data->indirect.Length(); j++) {
    ...
    int64_t indirectBlock = fVolume->ToBlock(data->double_indirect);

	for (int32_t l = 0; l < data->double_indirect.Length(); l++) {
		...
		for (int32_t k = 0; k < count; k++) {
			...
			for (int32_t j = 0; j < indirectRuns[k].Length(); j++) {
...
status_t Inode::Remove(Transaction &transaction, const char *name, int64_t *_id,
	bool isDirectory)
{
	BPlusTree *tree;
    ...
    int64_t id;
	if (tree->Find((uint8_t *)name, (uint16_t)strlen(name), &id) < B_OK)
		return B_ENTRY_NOT_FOUND;
	...
	// bit is set for indices in BEFS, not for attribute directories) - but you
	...
	// update the inode, so that no one will ever doubt it's deleted :-)
	inode->Node().flags |= HOST_ENDIAN_TO_BEFS_INT32(INODE_DELETED);
	inode->Node().flags &= ~HOST_ENDIAN_TO_BEFS_INT32(INODE_IN_USE);
...
status_t Inode::Create(Transaction &transaction, Inode *parent, const char *name,
    int32_t mode, int openMode, uint32_t type, int64_t *_id, Inode **_inode)
{
	FUNCTION_START(("name = %s, mode = %I64d\n", name, mode));
	...
	if (tree != NULL) {
        ...
        int64_t offset;
		if (tree->Find((uint8_t *)name, (uint16_t)strlen(name), &offset) == B_OK) {
			...
			Inode *inode;
            status_t l_status = vnode.Get(&inode);
            if (l_status < B_OK) {
                REPORT_ERROR(l_status);
				return B_ENTRY_NOT_FOUND;
			}
			...
			if (openMode & O_TRUNC) {
                ...
                l_status = inode->CheckPermissions(W_OK);
                if (l_status != B_OK)
                    return l_status;
                ...
                l_status = inode->SetFileSize(transaction, 0);
                if (l_status >= B_OK)
                    l_status = inode->WriteBack(transaction);
	...
	node->uid = HOST_ENDIAN_TO_BEFS_INT32(geteuid());
	node->gid = HOST_ENDIAN_TO_BEFS_INT32(parent
		? parent->Node().GroupID() : getegid());
		// the group ID is inherited from the parent, if available

	node->type = HOST_ENDIAN_TO_BEFS_INT32(type);
...
status_t AttributeIterator::Rewind()
{
	fCurrentSmallData = 0;
...
status_t AttributeIterator::GetNext(char *name, size_t *_length, uint32_t *_type,
//	vnode_id *_id)
...
void AttributeIterator::Update(uint16_t index, int8_t change)
{
