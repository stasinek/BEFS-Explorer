...
struct run_array {
	...
	int32_t CountRuns() const { return BEFS_ENDIAN_TO_HOST_INT32(count); }
	int32_t MaxRuns() const { return BEFS_ENDIAN_TO_HOST_INT32(max_runs) - 1; }
		// that -1 accounts for an off-by-one error in Be's BEFS implementation
	const block_run &RunAt(int32_t i) const { return runs[i]; }

	static int32_t MaxRuns(int32_t blockSize)
...
class RunArrays {
	public:
        ...
        status_t Insert(int64_t blockNumber);

		run_array *ArrayAt(int32_t i) { return fArrays.Array()[i]; }
		int32_t CountArrays() const { return fArrays.CountItems(); }
...
class LogEntry : public DoublyLinkedListLinkImpl<LogEntry> {
	public:
		LogEntry(Journal *journal, uint32_t logStart, uint32_t length);
		~LogEntry();

		uint32_t Start() const { return fStart; }
		uint32_t Length() const { return fLength; }
...
LogEntry::LogEntry(Journal *journal, uint32_t start, uint32_t length)
	:
	fJournal(journal),
	fStart(start),
	fLength(length)
...
{
	for (int32_t i = 0; i < CountArrays(); i++) {
		run_array *array = ArrayAt(i);

		for (int32_t j = 0; j < array->CountRuns(); j++) {
...
RunArrays::_AddRun(block_run &run)
{
	...
	// Be's BEFS log replay routine can only deal with block_runs of size 1
	...
	for (int32_t i = 0; i < CountArrays(); i++) {
		...
		for (int32_t j = 0; j < array->CountRuns(); j++) {
			...
			if (run.Start() == arrayRun.Start() + arrayRun.Length()) {
				// matches the end
				arrayRun.length = HOST_ENDIAN_TO_BEFS_INT16(arrayRun.Length() + 1);
				array->block_count++;
				fLength++;
				return true;
	...
	fLastArray->count = HOST_ENDIAN_TO_BEFS_INT16(fLastArray->CountRuns() + 1);
...
status_t
RunArrays::Insert(int64_t blockNumber)
{
	...
	if (!_AddRun(run)) {
		...
		fLastArray->count = HOST_ENDIAN_TO_BEFS_INT16(1);
		fLastArray->block_count++;
		fLength++;
	}
...
RunArrays::PrepareForWriting()
{
	...
	for (int32_t i = 0; i < CountArrays(); i++) {
		ArrayAt(i)->max_runs = HOST_ENDIAN_TO_BEFS_INT32(run_array::MaxRuns(blockSize));
	}
}
...
{
	int32_t maxRuns = run_array::MaxRuns(fVolume->BlockSize()) - 1;
		// the -1 works around an off-by-one bug in Be's BEFS implementation,
	...
	PRINT(("Log entry has %I64d entries\n", array->CountRuns()));
	return B_OK;
}
...
status_t
Journal::_ReplayRunArray(int32_t *_start)
{
    PRINT(("ReplayRunArray(start = %ld)\n", *_start));

    int64_t logOffset = fVolume->ToBlock(fVolume->Log());
    int64_t blockNumber = *_start % fLogSize;
	...
	for (int32_t index = 0; index < array->CountRuns(); index++) {
		const block_run &run = array->RunAt(index);
		PRINT(("replay block run %lu:%u:%u in log at %I64d!\n", run.AllocationGroup(),
			run.Start(), run.Length(), blockNumber));

        int64_t offset = fVolume->ToOffset(run);
		for (int32_t i = 0; i < run.Length(); i++) {
			const uint8_t *data = cached.SetTo(logOffset + blockNumber);
			if (data == NULL)
...
Journal::ReplayLog()
{
	...
	int32_t start = fVolume->LogStart();
	int32_t lastStart = -1;
	while (true) {
...
void
Journal::_blockNotify(int32_t transactionID, void *arg)
{
	LogEntry *logEntry = (LogEntry *)arg;

	PRINT(("Log entry %p has been finished, transaction ID = %I64d\n", logEntry, transactionID));
	...
	if (logEntry == journal->fEntries.First()) {
		...
		if (next != NULL) {
			int32_t length = next->Start() - logEntry->Start();
...
Journal::_WriteTransactionToLog()
{
	...
	int32_t blockShift = fVolume->BlockShift();
    int64_t logOffset = fVolume->ToBlock(fVolume->Log()) << blockShift;
    int64_t logStart = fVolume->LogEnd();
    int64_t logPosition = logStart % fLogSize;
	status_t status;
...
*/
	...
	for (int32_t k = 0; k < runArrays.CountArrays(); k++) {
		run_array *array = runArrays.ArrayAt(k);
		int32_t index = 0, count = 1;
		int32_t wrap = fLogSize - logStart;
...
{
	int32_t count = cache_blocks_in_transaction(fVolume->BlockCache(), fTransactionID);
	if (count < 0)
		return 0;
...
status_t
Journal::LogBlocks(int64_t blockNumber, const uint8_t *buffer, size_t numBlocks)
{
...
status_t 
Transaction::Start(Volume *volume, int64_t refBlock)
{
