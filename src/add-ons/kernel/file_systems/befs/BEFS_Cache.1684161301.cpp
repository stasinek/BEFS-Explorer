...
static uint32_t
transaction_hash2(void *_transaction, const void *_id, uint32_t range)
{
...
block_cache::block_cache(HANDLE _fd, int64_t numBlocks, size_t blockSize,
    bool readOnly)
    :
    hash(NULL),
    fd(_fd),
...
static void
add_to_iovec(iovec *vecs, int32_t &index, int32_t max, const void *address, size_t size)
{
...
cached_block *
block_cache::NewBlock(int64_t blockNumber)
{
...
{
    //TODO:
    //TRACE(("block_cache: remove up to %I64d unused blocks\n", count));
    printf("block_cache: remove up to %I64d unused blocks\n", count);
    cached_block *block;
    for (block_list::Iterator it = unused_blocks.GetIterator();
        ...
        if (maxAccessed < block->accessed)
            continue;
        //TODO:
        //TRACE(("  remove block %I64d, accessed %I64d times\n",
        //	block->block_number, block->accessed));
        printf("  remove block %I64d, accessed %I64d times\n",
            block->block_number, block->accessed);
...
void *
block_cache_create(HANDLE fd, int64_t numBlocks, size_t blockSize, bool readOnly)
{
...
/* extern */ status_t	block_cache_make_writable(void *_cache, int64_t blockNumber, int32_t transaction){
    printf("block_cache_make_writable not implemented\n");
    return 0;
};
/* extern */ void *		block_cache_get_writable_etc(void *_cache,int64_t blockNumber, int64_t base,int64_t length, int32_t transaction){
    printf("block_cache_get_writable_etc not implemented\n");
    return 0;
};
/* extern */ void *		block_cache_get_writable(void *_cache,int64_t blockNumber, int32_t transaction){
    printf("block_cache_get_writable not implemented\n");
    return 0;
};
/* extern */ void *		block_cache_get_empty(void *_cache,int64_t blockNumber, int32_t transaction){
    printf("block_cache_get_empty not implemented\n");
    return 0;
};
/*  const void * block_cache_get_etc(void *_cache, int64_t blockNumber, int64_t base, int64_t length){
    printf("block_cache_get_etc not implemented\n");
    return 0;
};
const void * block_cache_get(void *_cache,	int64_t blockNumber){
...
/*!
    ...
    \param readBlock if \c false, the block will not be read in case it was
        ...
        data.
*/
static cached_block *get_cached_block(block_cache *cache, int64_t blockNumber, bool *_allocated,/*int64_t offset,*/bool readBlock = true)
{
...
// how to get volume->StartPos() (=2664990720) here???????
        //extern int64_t Myoffset=2664990720;
        ...
        if (read_pos(cache->fd, /*2664990720+*/blockNumber * blockSize, block->current_data,
            ...
            //}
            //TODO:
            //FATAL(("could not read block %I64d\n", blockNumber));
            printf("could not read block %I64d\n", blockNumber);
            return NULL;
        }
    }//else printf("no\n");

    if (block->unused) {
        //TRACE(("remove block %I64d from unused\n", blockNumber));
        block->unused = false;
...
const void *
block_cache_get_etc(void *_cache, int64_t blockNumber, int64_t base, int64_t length/*, int64_t offset*/)
{
    ...
    cached_block *block = get_cached_block(cache, blockNumber, &allocated, 0 /*offset*/);
    if (block == NULL){
        printf("block == NULL\n");
        return NULL;
    }
...
const void *
block_cache_get(void *_cache, int64_t blockNumber)
{
    return block_cache_get_etc(_cache, blockNumber, blockNumber, 1/*,offset*/);
}

/* extern */ status_t	block_cache_set_dirty(void *_cache, int64_t blockNumber, bool isDirty,int32_t transaction){
...
void
block_cache_put(void *_cache, int64_t blockNumber)
{
    block_cache *cache = (block_cache *)_cache;
    //TODO: benaphore stuff
    //BenaphoreLocker locker(&cache->lock);
...
void vm_cache_insert_page(file_cache_ref *cache, vm_page *page,int64_t offset){
...
vm_page *
vm_cache_lookup_page(file_cache_ref *cache, int64_t offset)
{
...
void *
file_cache_create(mount_id mountID, vnode_id vnodeID, int64_t size, HANDLE fd, Volume* v)
{
    printf("file_cache_create(mountID = %I64d, vnodeID = %I64d, size = %I64d, fd = 0x%p)\n", mountID, vnodeID, size, fd);
    ...
    // create lock
    char buffer[32];
    //snprintf(buffer, sizeof(buffer), "file cache %I64d:%lld", mountID, vnodeID);
    //TODO: unsafe, should be snprintf
    sprintf(buffer, "file cache %I64d:%lld", mountID, vnodeID);
    //TODO: enable locking:
...
status_t
vfs_read_pages(HANDLE fd, int64_t pos, const iovec *vecs, size_t count,
    int64_t *_numBytes, bool fsReenter)
{
...
status_t vfs_write_pages(HANDLE fd, int64_t pos, const iovec *vecs, size_t count,
    int64_t *_numBytes, bool fsReenter){
...
file_extent *
find_file_extent(file_cache_ref *ref, int64_t offset, uint32_t *_index)
{
...
status_t get_file_map(file_cache_ref* ref, int64_t offset, int64_t size,
    file_io_vec *vecs, size_t *_count)
{
    ...
    int32_t blockShift = volume->BlockShift();
...
//status_t pages_io(file_cache_ref *ref, int64_t offset, const iovec *vecs,
...
status_t
pages_io(file_cache_ref *ref, int64_t offset, const iovec *vecs, size_t count,
    int64_t *_numBytes, bool doWrite)
{
...
//#ifdef TRACE_FILE_CACHE
    ...
    for (size_t ic = 0; ic < fileVecCount; ic++) {
        printf("  [%lu] offset = %I64d, size = %I64d\n",
            ic, fileVecs[ic].offset, fileVecs[ic].length);
    }
//#endif
    ...
    if (fileVecCount == 0) {
        ...
        printf("pages_io: access outside of vnode %p at offset %I64d\n",
    ...
    while (true) {
        for (int i; fileVecIndex < fileVecCount; fileVecIndex++) {
            file_io_vec &fileVec = fileVecs[fileVecIndex];
            int64_t fileOffset = fileVec.offset;
            int64_t fileLeft = min_c(fileVec.length, bytesLeft);

            printf("FILE VEC [%lu] length %I64d\n", fileVecIndex, fileLeft);
            ...
            while (fileLeft > 0) {
                ...
                for (size = 0; size < fileLeft && i < count
                    ...
                    printf("fill vec %I64d, offset = %lu, size = %lu\n",
                        i, vecOffset, size);
...
//#ifdef TRACE_FILE_CACHE
            printf("got %lu file vecs for %I64d:%lu%s:\n", fileVecCount,
...
status_t
write_to_cache(file_cache_ref *ref, int64_t offset, size_t size, addr_t buffer, size_t bufferSize){
...
*/
inline status_t
read_chunk_into_cache(file_cache_ref *ref, int64_t offset, int64_t numBytes,
    int32_t pageOffset, addr_t buffer, int64_t bufferSize)
{
    printf("read_chunk(offset = %I64d, size =%I64d, pageOffset = %I64d, buffer = %p, bufferSize = %I64d\n",
    ...
    for (int64_t pos = 0; pos < numBytes; pos += B_PAGE_SIZE) {
    ...
    for (int32_t ip = pageIndex; ip-- > 0;) {
        vm_page_set_state(pages[ip], PAGE_STATE_ACTIVE);
        vm_page_put_page(pages[ip]);
    }
...
*/
status_t
read_into_cache(file_cache_ref *ref, int64_t offset, int64_t size, addr_t buffer, int64_t bufferSize)
{
...
status_t
satisfy_cache_io(file_cache_ref *ref, int64_t offset, addr_t buffer, addr_t lastBuffer,
    bool doWrite)
{
    int64_t requestSize = buffer - lastBuffer;
...
status_t
cache_io(void *_cacheRef, int64_t offset, addr_t buffer, int64_t *_size, bool doWrite)
{
    if (_cacheRef == NULL)
    ...
    int64_t fileSize = ref->virtual_size;
    ...
    int64_t bytesLeft = size, lastLeft = size;
    int32_t lastPageOffset = pageOffset;
    addr_t lastBuffer = buffer;
    int64_t lastOffset = offset;
...
printf("page1!=NULL\n");
        ...
        //TODO: test:
        int64_t bytesInPage = min_c(int64_t(B_PAGE_SIZE - pageOffset), bytesLeft);
        //int64_t bytesInPage = 4096;
...
cached_block::Compare(void *_cacheEntry, const void *_block)
{
    ...
    const int64_t *block = (const int64_t *)_block;
...
status_t
file_map::Add(file_io_vec *vecs, size_t vecCount, int64_t &lastOffset)
{
    printf("file_map::Add(vecCount = %I64d)\n", vecCount);
...
//#ifdef TRACE_FILE_CACHE
    for (uint32_t ie = 0; ie < count; ie++) {
        file_extent *extent = ExtentAt(ie);
        printf("file_map::Add: [%I64d] extend offset %I64d, disk offset %I64d, length %I64d\n",
            ie, extent->offset, extent->disk.offset, extent->disk.length);
    }
//#endif
...
vm_page::Compare(void *_cacheEntry, const void *_key)
{
    ...
    // device FD
    unsigned __int64 pageFD = reinterpret_cast<unsigned __int64>(page->cache->deviceFD);
    unsigned __int64 keyFD = reinterpret_cast<unsigned __int64>(key->deviceFD);
    if (pageFD != keyFD)
        return  pageFD - keyFD;
...
vm_page::Hash(void *_cacheEntry, const void *_key, uint32_t range)
{
    ...
    int64_t offset = (page ? page->offset : key->offset);

    //TODO: how the hell are we gonna fix this??  fixed now?
    //unsigned long value = (void*)fd;
    unsigned __int64 value = reinterpret_cast<unsigned __int64>(fd);
    value = value * 17 + id;
