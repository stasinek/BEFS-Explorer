...
struct disk_super_block {
	...
	uint64 NumBlocks() const { return BFS_ENDIAN_TO_HOST_INT64(num_blocks); }
	uint64 UsedBlocks() const { return BFS_ENDIAN_TO_HOST_INT64(used_blocks); }
	...
	uint64 LogStart() const { return BFS_ENDIAN_TO_HOST_INT64(log_start); }
	uint64 LogEnd() const { return BFS_ENDIAN_TO_HOST_INT64(log_end); }

	// implemented in Volume.cpp:
	bool IsValid();
	void Initialize(const char *name, uint64 numBlocks, uint32 blockSize);
}/* _PACKED*/;
...
struct data_stream {
	...
	uint64		max_direct_range;
	block_run	indirect;
	uint64		max_indirect_range;
	block_run	double_indirect;
	uint64		max_double_indirect_range;
	uint64		size;

	uint64 MaxDirectRange() const { return BFS_ENDIAN_TO_HOST_INT64(max_direct_range); }
	uint64 MaxIndirectRange() const { return BFS_ENDIAN_TO_HOST_INT64(max_indirect_range); }
	uint64 MaxDoubleIndirectRange() const { return BFS_ENDIAN_TO_HOST_INT64(max_double_indirect_range); }
	uint64 Size() const { return BFS_ENDIAN_TO_HOST_INT64(size); }
}/* _PACKED*/;
...
{
	// rounds up to the next uint64 boundary
	return (data + sizeof(uint64) - 1) & ~(sizeof(uint64) - 1);
}
