...
class RunArrays {
	public:
		...
		status_t Insert(uint64 blockNumber);
...
status_t
RunArrays::Insert(uint64 blockNumber)
{
...
Journal::_ReplayRunArray(int32 *_start)
{
	...
	uint64 logOffset = fVolume->ToBlock(fVolume->Log());
	uint64 blockNumber = *_start % fLogSize;
...
Journal::_WriteTransactionToLog()
{
	...
	uint64 logOffset = fVolume->ToBlock(fVolume->Log()) << blockShift;
	uint64 logStart = fVolume->LogEnd();
	uint64 logPosition = logStart % fLogSize;
	status_t status;
...
status_t
Journal::LogBlocks(uint64 blockNumber, const uint8 *buffer, size_t numBlocks)
{
...
status_t 
Transaction::Start(Volume *volume, uint64 refBlock)
{
