...
Index::UpdateSize(Transaction &transaction, Inode *inode)
{
	...
	uint64 oldSize = inode->OldSize();
	uint64 newSize = inode->Size();
...
Index::InsertLastModified(Transaction &transaction, Inode *inode)
{
	...
	uint64 modified = inode->LastModified();
...
Index::RemoveLastModified(Transaction &transaction, Inode *inode)
{
	...
	uint64 modified = inode->OldLastModified();
	...
	uint64 modified)
{
	...
	uint64 oldModified = inode->OldLastModified();
	if (modified == -1)
