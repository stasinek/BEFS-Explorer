...
block_cache::block_cache(HANDLE _fd, uint64 numBlocks, size_t blockSize,
	bool readOnly)
	:
	hash(NULL),
	fd(_fd),
...
cached_block *
block_cache::NewBlock(uint64 blockNumber)
{
...
/*void *		block_cache_create(HANDLE fd, uint64 numBlocks, size_t blockSize, bool readOnly){
	printf("block_cache_create not implemented\n");
	return 0;
};*/
void *
block_cache_create(HANDLE fd, uint64 numBlocks, size_t blockSize, bool readOnly)
{
...
status_t	block_cache_sync_etc(void *_cache, uint64 blockNumber, size_t numBlocks){
	printf("block_cache_sync_etc not implemented\n");
	return 0;
};
/* extern */ status_t	block_cache_make_writable(void *_cache, uint64 blockNumber, int32_t transaction){
	printf("block_cache_make_writable not implemented\n");
	return 0;
};
/* extern */ void *		block_cache_get_writable_etc(void *_cache,uint64 blockNumber, uint64 base,uint64 length, int32_t transaction){
	printf("block_cache_get_writable_etc not implemented\n");
	return 0;
};
/* extern */ void *		block_cache_get_writable(void *_cache,uint64 blockNumber, int32_t transaction){
	printf("block_cache_get_writable not implemented\n");
	return 0;
};
/* extern */ void *		block_cache_get_empty(void *_cache,uint64 blockNumber, int32_t transaction){
	printf("block_cache_get_empty not implemented\n");
	return 0;
};
/*  const void * block_cache_get_etc(void *_cache, uint64 blockNumber, uint64 base, uint64 length){
	printf("block_cache_get_etc not implemented\n");
	return 0;
};
const void * block_cache_get(void *_cache,	uint64 blockNumber){
...
// how to get volume->StartPos() (=2664990720) here???????
		//extern uint64 Myoffset=2664990720;
...
const void *
block_cache_get_etc(void *_cache, uint64 blockNumber, uint64 base, uint64 length/*, uint64 offset*/)
{
...
const void *
block_cache_get(void *_cache, uint64 blockNumber)
{
	return block_cache_get_etc(_cache, blockNumber, blockNumber, 1/*,offset*/);
}

/* extern */ status_t	block_cache_set_dirty(void *_cache, uint64 blockNumber, bool isDirty,int32_t transaction){
	printf("block_cache_set_dirty not implemented\n");
	return 0;
};
/* void		block_cache_put(void *_cache,uint64 blockNumber){
	printf("block_cache_put not implemented\n");
}; */
void
block_cache_put(void *_cache, uint64 blockNumber)
{
	block_cache *cache = (block_cache *)_cache;
	//TODO: benaphore stuff
	//BenaphoreLocker locker(&cache->lock);
...
void vm_cache_insert_page(file_cache_ref *cache, vm_page *page,uint64 offset){ 
...
status_t	file_cache_set_size(void *_cacheRef,uint64 size){
...
status_t
vfs_read_pages(HANDLE fd, uint64 pos, const iovec *vecs, size_t count,
...
//#endif
	...
	while (true) {
		for (; fileVecIndex < fileVecCount; fileVecIndex++) {
			...
			uint64 fileOffset = fileVec.offset;
			uint64 fileLeft = min_c(fileVec.length, bytesLeft);
...
status_t	file_cache_read(void *_cacheRef, uint64 offset,void *bufferBase, uint64 *_size){
...
status_t	file_cache_write(void *_cacheRef,uint64 offset, const void *buffer,size_t *_size){
...
status_t
write_to_cache(file_cache_ref *ref, uint64 offset, size_t size, addr_t buffer, size_t bufferSize){
	...
	int32 pageOffset, addr_t buffer, uint64 bufferSize)
{
	...
	for (uint64 pos = 0; pos < numBytes; pos += B_PAGE_SIZE) {
...
status_t
satisfy_cache_io(file_cache_ref *ref, uint64 offset, addr_t buffer, addr_t lastBuffer,
	bool doWrite)
{
	uint64 requestSize = buffer - lastBuffer;
...
status_t
cache_io(void *_cacheRef, uint64 offset, addr_t buffer, uint64 *_size, bool doWrite)
{
	if (_cacheRef == NULL)
	...
	uint64 bytesLeft = size, lastLeft = size;
...
cached_block::Compare(void *_cacheEntry, const void *_block)
{
	...
	const uint64 *block = (const uint64 *)_block;
...
status_t
file_map::Add(file_io_vec *vecs, size_t vecCount, uint64 &lastOffset)
{
...
vm_page::Hash(void *_cacheEntry, const void *_key, uint32 range)
{
	...
	uint64 offset = (page ? page->offset : key->offset);
