...
class Inode {
	public:
		...
		uint64 BlockNumber() const { return fVolume->VnodeToBlock(fID); }
		...
		uint64 Size() const { return fNode.data.Size(); }
		uint64 LastModified() const { return fNode.last_modified_time; }
		...
		status_t ReadAttribute(const char *name, int32 type, uint64 pos, uint8 *buffer, uint64 *_length);
		status_t WriteAttribute(Transaction &transaction, const char *name, int32 type, uint64 pos, const uint8 *buffer, size_t *_length);
		...
		status_t FindBlockRun(uint64 pos, block_run &run, uint64 &offset);

		status_t ReadAt(uint64 pos, uint8 *buffer, uint64 *length);
		status_t WriteAt(Transaction &transaction, uint64 pos, const uint8 *buffer, size_t *length);
		status_t FillGapWithZeros(uint64 oldSize, uint64 newSize);

		status_t SetFileSize(Transaction &transaction, uint64 size);
		status_t Append(Transaction &transaction, uint64 bytes);
		...
		status_t Remove(Transaction &transaction, const char *name, uint64 *_id = NULL,
					bool isDirectory = false);
		static status_t Create(Transaction &transaction, Inode *parent, const char *name,
					int32 mode, int omode, uint32 type, uint64 *_id = NULL, Inode **_inode = NULL);
		...
		uint64 OldSize() { return fOldSize; }
		uint64 OldLastModified() { return fOldLastModified; }
	...
	private:
		...
		status_t _GrowStream(Transaction &transaction, uint64 size);
		status_t _ShrinkStream(Transaction &transaction, uint64 size);
	...
	private:
		...
		void			*fCache;
		bfs_inode		fNode;
		uint64			fOldSize;			// we need those values to ensure we will remove
		uint64			fOldLastModified;	// the correct keys from the indices
